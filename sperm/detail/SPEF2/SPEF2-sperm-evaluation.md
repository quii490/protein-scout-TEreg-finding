---
type: sperm-protein-evaluation
gene: "SPEF2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# SPEF2 — 精子模块评估

## 1. 基本信息
- **基因:** SPEF2
- **Ensembl:** ENSG00000152582
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 75 篇
- **精子定位部位:** Flagellar centriole、Mid piece (2 个)
- **UniProt Subcellular Location:** GO: cilium; cytoplasm; extracellular region; Golgi apparatus; manchette

## 2. HPA 精子定位证据
- **来源:** Flagellar centriole、Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000152582-SPEF2
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/40343/2185_D3_56_blue_red_green.jpg)
![](https://images.proteinatlas.org/40343/2202_B3_21_blue_red_green.jpg)
![](https://images.proteinatlas.org/40343/2186_B2_60_blue_red_green.jpg)
![](https://images.proteinatlas.org/40343/2202_B3_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/40343/2185_D3_28_blue_red_green.jpg)
![](https://images.proteinatlas.org/40343/2206_G4_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H0YAC0 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Sperm_flagellar-bone_growth。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 75 篇 (较多研究量)
- 1. PMID 20301301: Primary Ciliary Dyskinesia. (1993) **
2. PMID 34932939: Bi-allelic variants in DNHD1 cause flagellar axoneme defects and asthenoteratozoospermia in humans and mice. (2022 Jan 6) *Am J Hum Genet*
3. PMID 34755699: Sperm flagellar 2 (SPEF2) is essential for sperm flagellar assembly in humans. (2022 Jul-Aug) *Asian J Androl*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H0YAC0)。参见 https://alphafold.ebi.ac.uk/entry/H0YAC0
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| ADK | 0.930 | 0.000 | 0.000 | 0.000 |
| SPAG6 | 0.925 | 0.000 | 0.000 | 0.000 |
| IFT20 | 0.791 | 0.000 | 0.000 | 0.000 |
| CFAP43 | 0.788 | 0.000 | 0.000 | 0.000 |
| SPAG6 | 0.786 | 0.000 | 0.000 | 0.000 |
| CFAP54 | 0.744 | 0.000 | 0.000 | 0.000 |
| TTC21A | 0.741 | 0.000 | 0.000 | 0.000 |
| CFAP43 | 0.735 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.724 | 0.000 | 0.000 | 0.000 |
| ARMC2 | 0.723 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| APOA1 | anti bait coimmunoprecipitation | 15174051 |
| IGHA1 | anti bait coimmunoprecipitation | 15174051 |
| KATNAL1 | tandem affinity purification | 26929214 |
| DISC1 | two hybrid fragment pooling approach | 31413325 |
| CLU | two hybrid fragment pooling approach | 31413325 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Flagellar centriole、Mid piece |
| PubMed | 6/20 | 75 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Flagellar centriole、Mid piece
- 建议验证精子 IF 文献定位
