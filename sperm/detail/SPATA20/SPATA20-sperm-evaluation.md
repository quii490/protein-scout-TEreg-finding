---
type: sperm-protein-evaluation
gene: "SPATA20"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# SPATA20 — 精子模块评估

## 1. 基本信息
- **基因:** SPATA20
- **Ensembl:** ENSG00000006282
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 21 篇
- **精子定位部位:** Principal piece (1 个)
- **UniProt Subcellular Location:** GO: extracellular region; mitochondrion

## 2. HPA 精子定位证据
- **来源:** Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000006282-SPATA20
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/27144/2239_D4_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/27144/2239_D4_57_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8PGR5 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 21 篇 (低研究量)
- 1. PMID 39417902: Genetic etiological spectrum of sperm morphological abnormalities. (2024 Nov) *J Assist Reprod Genet*
2. PMID 40268954: UBL7 is indispensable for spermiogenesis through protecting critical factors from excessive degradation by proteasomes. (2025 Apr 23) *Nat Commun*
3. PMID 28582417: Genome-wide alteration in DNA hydroxymethylation in the sperm from bisphenol A-exposed men. (2017) *PLoS One*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8PGR5)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8PGR5
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| TYMS | 0.825 | 0.000 | 0.000 | 0.000 |
| HMBS | 0.806 | 0.000 | 0.000 | 0.000 |
| SPATA17 | 0.738 | 0.000 | 0.000 | 0.000 |
| TYMS | 0.657 | 0.000 | 0.000 | 0.000 |
| NDUFA10 | 0.604 | 0.000 | 0.000 | 0.000 |
| SPATA6 | 0.594 | 0.000 | 0.000 | 0.000 |
| PHYKPL | 0.570 | 0.000 | 0.000 | 0.000 |
| ABAT | 0.567 | 0.000 | 0.000 | 0.000 |
| SPATA4 | 0.532 | 0.000 | 0.000 | 0.000 |
| SPATA46 | 0.527 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| XRN1 | two hybrid | 15231747 |
| CORO1A | two hybrid array | 21988832 |
| CCL25 | anti tag coimmunoprecipitation | 28514442 |
| TRUB2 | anti tag coimmunoprecipitation | 27499296 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Principal piece |
| PubMed | 8/20 | 21 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Principal piece
- 建议验证精子 IF 文献定位
