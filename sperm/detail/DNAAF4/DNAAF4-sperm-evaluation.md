---
type: sperm-protein-evaluation
gene: "DNAAF4"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DNAAF4 — 精子模块评估

## 1. 基本信息
- **基因:** DNAAF4
- **Ensembl:** ENSG00000256061
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 83 篇
- **精子定位部位:** Connecting piece、Mid piece、Principal piece (3 个)
- **UniProt Subcellular Location:** GO: cytoplasm; cytosol; dynein axonemal particle; extracellular region; neuron projection

## 2. HPA 精子定位证据
- **来源:** Connecting piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000256061-DNAAF4
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/51048/2240_D6_29_blue_red_green.jpg)
![](https://images.proteinatlas.org/51048/2240_D6_57_blue_red_green.jpg)
![](https://images.proteinatlas.org/70577/2234_F11_63_blue_red_green.jpg)
![](https://images.proteinatlas.org/70577/2234_F11_21_blue_red_green.jpg)
![](https://images.proteinatlas.org/51048/781_A6_8_blue_red_green.jpg)
![](https://images.proteinatlas.org/70577/2101_G6_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: E5RJ13 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Dynein_assembly_factor_4, HSP20-like_chaperone。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 83 篇 (较多研究量)
- 1. PMID 20301301: Primary Ciliary Dyskinesia. (1993) **
2. PMID 35903363: Case Report: DNAAF4 Variants Cause Primary Ciliary Dyskinesia and Infertility in Two Han Chinese Families. (2022) *Front Genet*
3. PMID 35873488: Strongly Truncated Dnaaf4 Plays a Conserved Role in Drosophila Ciliary Dynein Assembly as Part of an R2TP-Like Co-Chaperone Complex With Dnaaf6. (2022) *Front Genet*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: E5RJ13)。参见 https://alphafold.ebi.ac.uk/entry/E5RJ13
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DCDC2 | 0.986 | 0.000 | 0.000 | 0.000 |
| DCDC2 | 0.978 | 0.000 | 0.000 | 0.000 |
| KIAA0319 | 0.972 | 0.000 | 0.000 | 0.000 |
| PIH1D3 | 0.961 | 0.000 | 0.000 | 0.000 |
| DNAAF1 | 0.939 | 0.000 | 0.000 | 0.000 |
| LRRC6 | 0.938 | 0.000 | 0.000 | 0.000 |
| PIH1D3 | 0.910 | 0.000 | 0.000 | 0.000 |
| DNAAF3 | 0.901 | 0.000 | 0.000 | 0.000 |
| DNAAF3 | 0.886 | 0.000 | 0.000 | 0.000 |
| DNAAF4 | 0.883 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| MM46 | anti tag coimmunoprecipitation | 20562859 |
| GABARAPL1 | pull down | 20562859 |
| GABARAPL2 | pull down | 20562859 |
| MAP1LC3B | pull down | 20562859 |
| MAP1LC3C | pull down | 20562859 |
| MM46 | anti tag coimmunoprecipitation | 20562859 |
| GABARAPL1 | anti tag coimmunoprecipitation | 20562859 |
| MM46 | anti tag coimmunoprecipitation | 20562859 |
| CALM1 | inference by socio-affinity scoring |  |
| DNAAF2 | inference by socio-affinity scoring |  |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: Connecting piece、Mid piece、Principal piece |
| PubMed | 6/20 | 83 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Connecting piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
