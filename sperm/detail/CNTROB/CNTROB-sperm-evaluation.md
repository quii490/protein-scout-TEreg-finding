---
type: sperm-protein-evaluation
gene: "CNTROB"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CNTROB — 精子模块评估

## 1. 基本信息
- **基因:** CNTROB
- **Ensembl:** ENSG00000170037
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 33 篇
- **精子定位部位:** Flagellar centriole (1 个)
- **UniProt Subcellular Location:** GO: centriole; centrosome; ciliary basal body

## 2. HPA 精子定位证据
- **来源:** Flagellar centriole ✓
- **链接:** https://www.proteinatlas.org/ENSG00000170037-CNTROB
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/23321/194_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/23321/194_A9_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/23321/193_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/23321/192_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/23321/2121_D12_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/23321/2132_B6_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8V5D7 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 33 篇 (中等研究量)
- 1. PMID 40711600: A missense mutation in TCFL5 is associated with male infertility due to oligoasthenoteratozoospermia. (2025 Jul 25) *Mol Genet Genomics*
2. PMID 24432614: [Molecular genetic mechanisms of teratozoospermia]. (2013 Dec) *Zhonghua Nan Ke Xue*
3. PMID 31589365: The dynamics of gene expression during and post meiosis sets the sperm agenda. (2019 Dec) *Mol Reprod Dev*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8V5D7)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8V5D7
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| SASS6 | 0.999 | 0.000 | 0.000 | 0.000 |
| NIN | 0.979 | 0.000 | 0.000 | 0.000 |
| CNTLN | 0.969 | 0.000 | 0.000 | 0.000 |
| ODF2 | 0.969 | 0.000 | 0.000 | 0.000 |
| CCP110 | 0.964 | 0.000 | 0.000 | 0.000 |
| CCP110 | 0.961 | 0.000 | 0.000 | 0.000 |
| CNTROB | 0.902 | 0.000 | 0.000 | 0.000 |
| NIN | 0.898 | 0.000 | 0.000 | 0.000 |
| NIN | 0.894 | 0.000 | 0.000 | 0.000 |
| CETN2 | 0.892 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Flagellar centriole |
| PubMed | 7/20 | 33 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Flagellar centriole
- 建议验证精子 IF 文献定位
