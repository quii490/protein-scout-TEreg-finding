---
type: sperm-protein-evaluation
gene: "CCDC66"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CCDC66 — 精子模块评估

## 1. 基本信息
- **基因:** CCDC66
- **Ensembl:** 
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 42 篇
- **精子定位部位:** End piece、Perinuclear theca (2 个)
- **UniProt Subcellular Location:** GO: axoneme; centriolar satellite; centrosome; ciliary basal body; cilium

## 2. HPA 精子定位证据
- **来源:** End piece、Perinuclear theca ✓
- **链接:** https://www.proteinatlas.org/-CCDC66
- **IF 图像:** 未获取

*HPA IF 图像未获取。已查询 HPA subcellular 页面。*


## 3. UniProt / GO-CC 精子定位证据
**UniProt Subcellular Location:** GO: axoneme; centriolar satellite; centrosome; ciliary basal body; cilium

*GO-CC 精子相关条目待进一步查询。*

## 4. PubMed 文献证据
- **文献数:** 42 篇 (中等研究量)
- 1. PMID 42039574: Togaram Ensures Axial Alignment of the Sperm Neck. (2026 Apr 17) *bioRxiv*

## 5. AlphaFold / PAE / PDB / 结构域
*待结构数据采集。*
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| XRN2 | 0.999 | 0.000 | 0.000 | 0.000 |
| CEP290 | 0.956 | 0.000 | 0.000 | 0.000 |
| PCM1 | 0.909 | 0.000 | 0.000 | 0.000 |
| CCDC14 | 0.831 | 0.000 | 0.000 | 0.000 |
| CCDC14 | 0.811 | 0.000 | 0.000 | 0.000 |
| CEP290 | 0.797 | 0.000 | 0.000 | 0.000 |
| CEP290 | 0.795 | 0.000 | 0.000 | 0.000 |
| CCDC14 | 0.751 | 0.000 | 0.000 | 0.000 |
| CCDC66 | 0.705 | 0.000 | 0.000 | 0.000 |
| CCDC66 | 0.657 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| PCM1 | proximity-dependent biotin identification | 26638075 |
| CEP162 | proximity-dependent biotin identification | 26638075 |
| CEP135 | proximity-dependent biotin identification | 26638075 |
| C14orf145 | proximity-dependent biotin identification | 26638075 |
| PCM1 | proximity-dependent biotin identification | 26638075 |
| CEP63 | proximity-dependent biotin identification | 26638075 |
| CCDC123 | proximity-dependent biotin identification | 26638075 |
| ODF2 | proximity-dependent biotin identification | 26638075 |
| CCDC52 | proximity-dependent biotin identification | 26638075 |
| CCDC100 | proximity-dependent biotin identification | 26638075 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: End piece、Perinuclear theca |
| PubMed | 7/20 | 42 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | 待采集 |
| 新颖性 | 6/10 | 中等 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Perinuclear theca
- 建议验证精子 IF 文献定位
