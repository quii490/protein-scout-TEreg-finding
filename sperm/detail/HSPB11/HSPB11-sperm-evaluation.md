---
type: sperm-protein-evaluation
gene: "HSPB11"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# HSPB11 — 精子模块评估

## 1. 基本信息
- **基因:** HSPB11
- **蛋白名称:** Intraflagellar transport protein 25 homolog
- **Ensembl:** ENSG00000081870
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 41 篇
- **精子定位部位:** End piece、Mid piece、Principal piece (3 个)
- **UniProt Subcellular Location:** Cell projection, cilium

## 2. HPA 精子定位证据
- **来源:** End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000081870-HSPB11
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/30140/2229_G7_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/30140/2229_G7_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q9Y547 — Intraflagellar transport protein 25 homolog.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: FA58C, Galactose-bd-like_sf, IFT25。
Pfam: F5_F8_type_C。

## 4. PubMed 文献证据
- **文献数:** 41 篇 (中等研究量)
- 1. PMID 30709917: IFT25 is required for the construction of the trypanosome flagellum. (2019 Feb 22) *J Cell Sci*
2. PMID 34888642: Impaired cooperation between IFT74/BBS22-IFT81 and IFT25-IFT27/BBS19 causes Bardet-Biedl syndrome. (2022 May 19) *Hum Mol Genet*
3. PMID 19253336: Characterization of mouse IFT complex B. (2009 Aug) *Cell Motil Cytoskeleton*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q9Y547)。参见 https://alphafold.ebi.ac.uk/entry/Q9Y547
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| IFT27 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT46 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT22 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT52 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT57 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT46 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT27 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT88 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT74 | 0.999 | 0.000 | 0.000 | 0.000 |
| IFT22 | 0.999 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| IFT25 | pull down | 27173435 |
| IFT25 | pull down | 27173435 |
| IFT25 | pull down | 27173435 |
| IFT27 | two hybrid pooling approach | 16189514 |
| VHL | anti bait coimmunoprecipitation | 17353931 |
| IFT25 | two hybrid array | 31515488 |
| IFT25 | two hybrid array | 32296183 |
| IFT27 | validated two hybrid | 32296183 |
| GORASP2 | validated two hybrid | 32296183 |
| IFT25 | inference by socio-affinity scoring |  |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: End piece、Mid piece、Principal piece |
| PubMed | 7/20 | 41 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **68/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
