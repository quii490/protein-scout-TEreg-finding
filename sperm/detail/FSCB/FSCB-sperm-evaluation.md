---
type: sperm-protein-evaluation
gene: "FSCB"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# FSCB — 精子模块评估

## 1. 基本信息
- **基因:** FSCB
- **Ensembl:** ENSG00000189139
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 15 篇
- **精子定位部位:** End piece、Flagellar centriole、Mid piece、Principal piece (4 个)
- **UniProt Subcellular Location:** Cell projection, cilium, flagellum

## 2. HPA 精子定位证据
- **来源:** End piece、Flagellar centriole、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000189139-FSCB
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/3546/2194_F1_16_blue_red_green.jpg)
![](https://images.proteinatlas.org/3546/2194_F1_11_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q5H9T9 — Fibrous sheath CABYR-binding protein.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: FSCB。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 15 篇 (低研究量)
- 1. PMID 23303679: Loss of R2D2 proteins ROPN1 and ROPN1L causes defects in murine sperm motility, phosphorylation, and fibrous sheath integrity. (2013 Feb) *Biol Reprod*
2. PMID 39651266: An updated reference genome sequence and annotation reveals gene losses and gains underlying naked mole-rat biology. (2024 Nov 28) *bioRxiv*
3. PMID 17855365: FSCB, a novel protein kinase A-phosphorylated calcium-binding protein, is a CABYR-binding partner involved in late steps of fibrous sheath biogenesis. (2007 Nov 23) *J Biol Chem*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q5H9T9)。参见 https://alphafold.ebi.ac.uk/entry/Q5H9T9
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| AKAP3 | 0.989 | 0.000 | 0.000 | 0.000 |
| PRKACB | 0.987 | 0.000 | 0.000 | 0.000 |
| PRKACG | 0.968 | 0.000 | 0.000 | 0.000 |
| PRKACG | 0.965 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.963 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.956 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.953 | 0.000 | 0.000 | 0.000 |
| ROPN1L | 0.942 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.916 | 0.000 | 0.000 | 0.000 |
| PRKACA | 0.624 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 4 部位: End piece、Flagellar centriole、Mid piece、Principal piece |
| PubMed | 8/20 | 15 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Flagellar centriole、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
