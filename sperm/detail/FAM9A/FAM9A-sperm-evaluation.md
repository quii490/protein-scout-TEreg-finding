---
type: sperm-protein-evaluation
gene: "FAM9A"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# FAM9A — 精子模块评估

## 1. 基本信息
- **基因:** FAM9A
- **蛋白名称:** Protein FAM9A
- **Ensembl:** ENSG00000183304
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 8 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** Nucleus, nucleolus

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000183304-FAM9A
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/56076/2205_F4_41_blue_red_green.jpg)
![](https://images.proteinatlas.org/56076/2205_F4_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q8IZU1 — Protein FAM9A.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: XLR/SYCP3。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 8 篇 (极低研究量)
- 1. PMID 37594251: Scrutinizing the human TEX genes in the context of human male infertility. (2024 Mar) *Andrology*
2. PMID 29789566: Transcriptome analysis of the adult human Klinefelter testis and cellularity-matched controls reveals disturbed differentiation of Sertoli- and Leydig cells. (2018 May 22) *Cell Death Dis*
3. PMID 12213195: A new gene family (FAM9) of low-copy repeats in Xp22.3 expressed exclusively in testis: implications for recombinations in this region. (2002 Sep) *Genomics*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q8IZU1)。参见 https://alphafold.ebi.ac.uk/entry/Q8IZU1
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| FAM9A | 0.905 | 0.000 | 0.000 | 0.000 |
| NOLC1 | 0.714 | 0.000 | 0.000 | 0.000 |
| OGFOD1 | 0.641 | 0.000 | 0.000 | 0.000 |
| OGFOD1 | 0.641 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.614 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.571 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.502 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.480 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.474 | 0.000 | 0.000 | 0.000 |
| FAM9A | 0.456 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ? | two hybrid prey pooling approach | 32296183 |
| THAP1 | two hybrid prey pooling approach | 32296183 |
| TRIM41 | two hybrid prey pooling approach | 32296183 |
| FAM9B | two hybrid array | 32296183 |
| HOMER1 | validated two hybrid | 32296183 |
| PICK1 | two hybrid array | 32296183 |
| RNF151 | two hybrid array | 32296183 |
| HOMER1 | two hybrid array | 32296183 |
| LCE1E | two hybrid array | 32296183 |
| PHC1 | two hybrid array | 32296183 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 10/20 | 8 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **69/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
