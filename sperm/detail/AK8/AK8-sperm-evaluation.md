---
type: sperm-protein-evaluation
gene: "AK8"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# AK8 — 精子模块评估

## 1. 基本信息
- **基因:** AK8
- **Ensembl:** ENSG00000165695
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 25 篇
- **精子定位部位:** End piece、Flagellar centriole、Mid piece、Principal piece (4 个)
- **UniProt Subcellular Location:** GO: 9+2 motile cilium; axoneme; cytoplasm; cytosol; sperm flagellum

## 2. HPA 精子定位证据
- **来源:** End piece、Flagellar centriole、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000165695-AK8
- **IF 图像:** 已获取 (4 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (4 张 blue_red_green)。
![](https://images.proteinatlas.org/21443/2169_A2_42_blue_red_green.jpg)
![](https://images.proteinatlas.org/21443/2238_G7_29_blue_red_green.jpg)
![](https://images.proteinatlas.org/21443/2169_A2_56_blue_red_green.jpg)
![](https://images.proteinatlas.org/21443/2238_G7_8_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8RN29 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 25 篇 (低研究量)
- 1. PMID 38761355: Adenylate kinase phosphate energy shuttle underlies energetic communication in flagellar axonemes. (2024 Aug) *Sci China Life Sci*
2. PMID 41055978: EFCAB10 anchors AK8 to the radial spoke for proper ciliary motility. (2025 Oct 14) *Proc Natl Acad Sci U S A*
3. PMID 24740601: Adenine nucleotide metabolism and a role for AMP in modulating flagellar waveforms in mouse sperm. (2014 Jun) *Biol Reprod*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8RN29)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8RN29
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| *STRING 无数据* | — | — | — | — |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 4 部位: End piece、Flagellar centriole、Mid piece、Principal piece |
| PubMed | 8/20 | 25 篇 |
| PPI | 5/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **57/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Flagellar centriole、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
