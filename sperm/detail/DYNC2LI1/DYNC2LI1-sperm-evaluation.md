---
type: sperm-protein-evaluation
gene: "DYNC2LI1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DYNC2LI1 — 精子模块评估

## 1. 基本信息
- **基因:** DYNC2LI1
- **蛋白名称:** Cytoplasmic dynein 2 light intermediate chain 1
- **Ensembl:** ENSG00000138036
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 26 篇
- **精子定位部位:** Annulus、Equatorial segment、Mid piece、Perinuclear theca、Principal piece (5 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton, cilium axoneme; Cytoplasm, cytoskeleton, cilium basal body; Cytoplasm, cytoskeleton, microtubule organizing center, centrosome

## 2. HPA 精子定位证据
- **来源:** Annulus、Equatorial segment、Mid piece、Perinuclear theca、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000138036-DYNC2LI1
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/62905/1273_E4_4_blue_red_green.jpg)
![](https://images.proteinatlas.org/62905/1233_A4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/62905/2123_D6_12_blue_red_green.jpg)
![](https://images.proteinatlas.org/62905/1236_A4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/62905/1236_A4_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/62905/2123_D6_48_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H7BYC8 — Cytoplasmic dynein 2 light intermediate chain 1.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: DYNC2LI1, Dynein_light_int_chain, P-loop_NTPase。
Pfam: DLIC。

## 4. PubMed 文献证据
- **文献数:** 26 篇 (低研究量)
- 1. PMID 26077881: Mutations in DYNC2LI1 disrupt cilia function and cause short rib polydactyly syndrome. (2015 Jun 16) *Nat Commun*
2. PMID 37769382: Comparative transcriptome analysis reveals key genes and pathways related to gonad development in the sea cucumber Apostichopus japonicus. (2023 Dec) *Comp Biochem Physiol Part D Genomics Proteomics*
3. PMID 36632779: Multiple interactions of the dynein-2 complex with the IFT-B complex are required for effective intraflagellar transport. (2023 Mar 1) *J Cell Sci*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H7BYC8)。参见 https://alphafold.ebi.ac.uk/entry/H7BYC8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DYNC1H1 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNC2H1 | 0.999 | 0.000 | 0.000 | 0.000 |
| WDR60 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNC2H1 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNC2LI1 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNC2H1 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNC2H1 | 0.999 | 0.000 | 0.000 | 0.000 |
| DYNLRB1 | 0.998 | 0.000 | 0.000 | 0.000 |
| WDR60 | 0.998 | 0.000 | 0.000 | 0.000 |
| DYNLL1 | 0.997 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 5 部位: Annulus、Equatorial segment、Mid piece、Perinuclear theca、Principal piece |
| PubMed | 8/20 | 26 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Annulus、Equatorial segment、Mid piece、Perinuclear theca、Principal piece
- 建议验证精子 IF 文献定位
