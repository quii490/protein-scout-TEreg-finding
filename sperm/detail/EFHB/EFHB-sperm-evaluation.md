---
type: sperm-protein-evaluation
gene: "EFHB"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# EFHB — 精子模块评估

## 1. 基本信息
- **基因:** EFHB
- **Ensembl:** ENSG00000163576
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 12 篇
- **精子定位部位:** Connecting piece (1 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton

## 2. HPA 精子定位证据
- **来源:** Connecting piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000163576-EFHB
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/34835/1702_G6_31_blue_red_green.jpg)
![](https://images.proteinatlas.org/34835/2238_B7_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/34835/2238_B7_15_blue_red_green.jpg)
![](https://images.proteinatlas.org/34835/1706_G9_13_cr6038c6dd010f7_blue_red_green.jpg)
![](https://images.proteinatlas.org/34835/1788_F5_29_cr5971d74b499c2_blue_red_green.jpg)
![](https://images.proteinatlas.org/34835/1788_F5_10_cr5971d74b481a3_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H7C0U4 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: EFHC1/EFHC2/EFHB。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 12 篇 (低研究量)
- 1. PMID 36896575: The human sperm proteome-Toward a panel for male fertility testing. (2023 Oct) *Andrology*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H7C0U4)。参见 https://alphafold.ebi.ac.uk/entry/H7C0U4
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| TUBA1A | 0.999 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.966 | 0.000 | 0.000 | 0.000 |
| CFAP45 | 0.949 | 0.000 | 0.000 | 0.000 |
| PACRG | 0.945 | 0.000 | 0.000 | 0.000 |
| EFHB | 0.944 | 0.000 | 0.000 | 0.000 |
| EFHC2 | 0.943 | 0.000 | 0.000 | 0.000 |
| PACRG | 0.929 | 0.000 | 0.000 | 0.000 |
| EFHC2 | 0.926 | 0.000 | 0.000 | 0.000 |
| ENKUR | 0.922 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.921 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Connecting piece |
| PubMed | 8/20 | 12 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Connecting piece
- 建议验证精子 IF 文献定位
