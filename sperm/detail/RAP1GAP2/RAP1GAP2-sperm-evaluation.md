---
type: sperm-protein-evaluation
gene: "RAP1GAP2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# RAP1GAP2 — 精子模块评估

## 1. 基本信息
- **基因:** RAP1GAP2
- **Ensembl:** ENSG00000132359
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 18 篇
- **精子定位部位:** Flagellar centriole、Perinuclear theca、Principal piece (3 个)
- **UniProt Subcellular Location:** GO: cytosol

## 2. HPA 精子定位证据
- **来源:** Flagellar centriole、Perinuclear theca、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000132359-RAP1GAP2
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/22896/236_B2_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/22896/237_B2_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/22896/237_B2_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/22896/2216_B9_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/22896/236_B2_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/22896/268_B2_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A1B0GV05 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Rap/Ran-GAP_sf, Rap/Ran_GAP_dom, Rap1_Ran_GAP。
Pfam: Rap-GAP_dimer, Rap_GAP。

## 4. PubMed 文献证据
- **文献数:** 18 篇 (低研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A1B0GV05)。参见 https://alphafold.ebi.ac.uk/entry/A0A1B0GV05
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| RAPGEF4 | 0.998 | 0.000 | 0.000 | 0.000 |
| RAPGEF4 | 0.994 | 0.000 | 0.000 | 0.000 |
| YWHAZ | 0.992 | 0.000 | 0.000 | 0.000 |
| RAPGEF3 | 0.987 | 0.000 | 0.000 | 0.000 |
| RAPGEF3 | 0.986 | 0.000 | 0.000 | 0.000 |
| RAPGEF3 | 0.986 | 0.000 | 0.000 | 0.000 |
| RAPGEF4 | 0.983 | 0.000 | 0.000 | 0.000 |
| RAPGEF4 | 0.983 | 0.000 | 0.000 | 0.000 |
| RAPGEF3 | 0.982 | 0.000 | 0.000 | 0.000 |
| PRKACG | 0.965 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| WNK1 | two hybrid pooling approach | 20936779 |
| KIAA0474 | anti tag coimmunoprecipitation | 33961781 |
| KIAA0474 | anti tag coimmunoprecipitation | 33961781 |
| VPS26C | two hybrid fragment pooling approach | 35914814 |
| GABARAPL2 | proximity-dependent biotin identification | 34524948 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: Flagellar centriole、Perinuclear theca、Principal piece |
| PubMed | 8/20 | 18 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **71/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Flagellar centriole、Perinuclear theca、Principal piece
- 建议验证精子 IF 文献定位
