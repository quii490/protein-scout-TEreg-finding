---
type: sperm-protein-evaluation
gene: "REEP6"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# REEP6 — 精子模块评估

## 1. 基本信息
- **基因:** REEP6
- **蛋白名称:** Receptor expression-enhancing protein
- **Ensembl:** ENSG00000115255
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 35 篇
- **精子定位部位:** Acrosome (1 个)
- **UniProt Subcellular Location:** Cytoplasmic vesicle, clathrin-coated vesicle membrane; Endoplasmic reticulum membrane; Membrane

## 2. HPA 精子定位证据
- **来源:** Acrosome ✓
- **链接:** https://www.proteinatlas.org/ENSG00000115255-REEP6
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/48015/1334_G7_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/48015/2204_G5_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/48015/846_A11_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/48015/1334_G7_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/48015/846_A11_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/48015/807_A11_2_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A1L5BXV2 — Receptor expression-enhancing protein.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: TB2_DP1_HVA22。
Pfam: TB2_DP1_HVA22。

## 4. PubMed 文献证据
- **文献数:** 35 篇 (中等研究量)
- 1. PMID 36306217: Testis-specific serine kinase 3 is required for sperm morphogenesis and male fertility. (2023 Jul) *Andrology*
2. PMID 32101290: Knockout of mouse receptor accessory protein 6 leads to sperm function and morphology defects†. (2020 May 26) *Biol Reprod*
3. PMID 37118964: Differential Proteomic Analysis of Human Sperm: A Systematic Review to Identify Candidate Targets to Monitor Sperm Quality. (2024 Jan) *World J Mens Health*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A1L5BXV2)。参见 https://alphafold.ebi.ac.uk/entry/A0A1L5BXV2
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| RTN3 | 0.993 | 0.000 | 0.000 | 0.000 |
| REEP5 | 0.954 | 0.000 | 0.000 | 0.000 |
| REEP5 | 0.906 | 0.000 | 0.000 | 0.000 |
| REEP5 | 0.902 | 0.000 | 0.000 | 0.000 |
| RTN4 | 0.849 | 0.000 | 0.000 | 0.000 |
| REEP5 | 0.835 | 0.000 | 0.000 | 0.000 |
| RTN1 | 0.832 | 0.000 | 0.000 | 0.000 |
| RTN4 | 0.817 | 0.000 | 0.000 | 0.000 |
| REEP5 | 0.735 | 0.000 | 0.000 | 0.000 |
| KLRF1 | 0.698 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Acrosome |
| PubMed | 7/20 | 35 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome
- 建议验证精子 IF 文献定位
