---
type: sperm-protein-evaluation
gene: "CXorf66"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CXorf66 — 精子模块评估

## 1. 基本信息
- **基因:** CXorf66
- **蛋白名称:** Uncharacterized protein CXorf66
- **Ensembl:** ENSG00000203933
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 2 篇
- **精子定位部位:** Annulus、Calyx、End piece、Mid piece、Perinuclear theca、Principal piece (6 个)
- **UniProt Subcellular Location:** Membrane

## 2. HPA 精子定位证据
- **来源:** Annulus、Calyx、End piece、Mid piece、Perinuclear theca、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000203933-CXorf66
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/48517/2212_C9_16_blue_red_green.jpg)
![](https://images.proteinatlas.org/48517/2212_C9_41_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q5JRM2 — Uncharacterized protein CXorf66.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: CXorf66。
Pfam: CXorf66。

## 4. PubMed 文献证据
- **文献数:** 2 篇 (极低研究量)
- 1. PMID 24709545: A novel transmembrane glycoprotein cancer biomarker present in the X chromosome. (2014 Mar-Apr) *Cancer Genomics Proteomics*
2. PMID 35164824: Identification of an SRY-negative 46,XX infertility male with a heterozygous deletion downstream of SOX3 gene. (2022 Feb 14) *Mol Cytogenet*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q5JRM2)。参见 https://alphafold.ebi.ac.uk/entry/Q5JRM2
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| RPS9 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPS23 | 0.999 | 0.000 | 0.000 | 0.000 |
| FAU | 0.999 | 0.000 | 0.000 | 0.000 |
| RPLP0 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPS3A | 0.999 | 0.000 | 0.000 | 0.000 |
| RPS26 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPL30 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPL11 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPL5 | 0.999 | 0.000 | 0.000 | 0.000 |
| RPL5 | 0.999 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 6 部位: Annulus、Calyx、End piece、Mid piece、Perinuclear theca、Principal piece |
| PubMed | 10/20 | 2 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **77/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Annulus、Calyx、End piece、Mid piece、Perinuclear theca、Principal piece
- 建议验证精子 IF 文献定位
