---
type: sperm-protein-evaluation
gene: "EPHA6"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# EPHA6 — 精子模块评估

## 1. 基本信息
- **基因:** EPHA6
- **蛋白名称:** Ephrin type-A receptor 6
- **Ensembl:** ENSG00000080224
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 92 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** Membrane

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000080224-EPHA6
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/7397/2217_D10_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/7397/2217_D10_26_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A0B4J1T8 — Ephrin type-A receptor 6.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: EPH-A6_SAM, Eph_TM, EphA6_rcpt_lig-bd, Ephrin_rcpt_lig-bd_dom, Ephrin_rcpt_TKs。
Pfam: EphA2_TM, Ephrin_CRD, Ephrin_lbd, Ephrin_rec_like, fn3。

## 4. PubMed 文献证据
- **文献数:** 92 篇 (较多研究量)
- 1. PMID 14726470: Differential gene expression of Eph receptors and ephrins in benign human tissues and cancers. (2004 Mar) *Clin Chem*
2. PMID 8892754: Cloning of m-ehk2 from the murine inner ear, an eph family receptor tyrosine kinase expressed in the developing and adult cochlea. (1996 Oct) *DNA Cell Biol*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A0B4J1T8)。参见 https://alphafold.ebi.ac.uk/entry/A0A0B4J1T8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| EPHA2 | 0.999 | 0.000 | 0.000 | 0.000 |
| EPHA2 | 0.999 | 0.000 | 0.000 | 0.000 |
| EFNA1 | 0.999 | 0.000 | 0.000 | 0.000 |
| EFNA4 | 0.997 | 0.000 | 0.000 | 0.000 |
| EFNA3 | 0.997 | 0.000 | 0.000 | 0.000 |
| EFNB2 | 0.973 | 0.000 | 0.000 | 0.000 |
| EFNA5 | 0.962 | 0.000 | 0.000 | 0.000 |
| ABL1 | 0.951 | 0.000 | 0.000 | 0.000 |
| EFNA1 | 0.950 | 0.000 | 0.000 | 0.000 |
| EPHA2 | 0.937 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| DEFA5 | anti tag coimmunoprecipitation | 28514442 |
| GPHA2 | anti tag coimmunoprecipitation | 28514442 |
| PROC | anti tag coimmunoprecipitation | 28514442 |
| GPHA2 | anti tag coimmunoprecipitation | 33961781 |
| DEFA5 | anti tag coimmunoprecipitation | 33961781 |
| EPHA2 | anti tag coimmunoprecipitation | 33961781 |
| RLN1 | anti tag coimmunoprecipitation | 33961781 |
| EFNA4 | anti tag coimmunoprecipitation | 33961781 |
| EFNA5 | anti tag coimmunoprecipitation | 33961781 |
| PROC | anti tag coimmunoprecipitation | 33961781 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 6/20 | 92 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **58/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
