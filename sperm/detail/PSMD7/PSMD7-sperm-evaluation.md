---
type: sperm-protein-evaluation
gene: "PSMD7"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# PSMD7 — 精子模块评估

## 1. 基本信息
- **基因:** PSMD7
- **蛋白名称:** 26S proteasome non-ATPase regulatory subunit 7
- **Ensembl:** ENSG00000103035
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 57 篇
- **精子定位部位:** Acrosome、Equatorial segment、Mid piece、Principal piece (4 个)
- **UniProt Subcellular Location:** GO: proteasome complex

## 2. HPA 精子定位证据
- **来源:** Acrosome、Equatorial segment、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000103035-PSMD7
- **IF 图像:** 未获取

*HPA IF 图像未获取。已查询 HPA subcellular 页面。*


## 3. UniProt / GO-CC 精子定位证据
UniProt: H3BTM8 — 26S proteasome non-ATPase regulatory subunit 7.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: JAMM/MPN+_dom, MPN。
Pfam: JAB。

## 4. PubMed 文献证据
- **文献数:** 57 篇 (中等研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H3BTM8)。参见 https://alphafold.ebi.ac.uk/entry/H3BTM8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| PSMD8 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMA3 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMC1 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMD7 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMB7 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMA2 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMC4 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMD11 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMC1 | 0.999 | 0.000 | 0.000 | 0.000 |
| PSMD8 | 0.999 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| NFKB2 | tandem affinity purification | 14743216 |
| TRAF6 | tandem affinity purification | 14743216 |
| RAB1A | two hybrid pooling approach | 16169070 |
| PSMD13 | anti bait coimmunoprecipitation | 17353931 |
| PAAF1 | anti bait coimmunoprecipitation | 17353931 |
| PSMD6 | anti bait coimmunoprecipitation | 17353931 |
| ADPG | anti bait coimmunoprecipitation | 17353931 |
| PSMC4 | anti bait coimmunoprecipitation | 17353931 |
| IKBKE | anti bait coimmunoprecipitation | 17353931 |
| Gankyrin | anti bait coimmunoprecipitation | 17353931 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 4 部位: Acrosome、Equatorial segment、Mid piece、Principal piece |
| PubMed | 7/20 | 57 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **69/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome、Equatorial segment、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
