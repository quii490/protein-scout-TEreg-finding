---
type: sperm-protein-evaluation
gene: "NBPF9"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# NBPF9 — 精子模块评估

## 1. 基本信息
- **基因:** NBPF9
- **Ensembl:** ENSG00000269713
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 4 篇
- **精子定位部位:** End piece、Mid piece、Principal piece (3 个)

## 2. HPA 精子定位证据
- **来源:** End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000269713-NBPF9
- **IF 图像:** 未获取

*HPA IF 图像未获取。已查询 HPA subcellular 页面。*


## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A087X0Y3 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: NBPF, Olduvai_dom。
Pfam: Olduvai。

## 4. PubMed 文献证据
- **文献数:** 4 篇 (极低研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A087X0Y3)。参见 https://alphafold.ebi.ac.uk/entry/A0A087X0Y3
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| NBPF9 | 0.595 | 0.000 | 0.000 | 0.000 |
| SPDYE5 | 0.483 | 0.000 | 0.000 | 0.000 |
| NBPF9 | 0.445 | 0.000 | 0.000 | 0.000 |
| TYW1B | 0.431 | 0.000 | 0.000 | 0.000 |
| NBPF9 | 0.422 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: End piece、Mid piece、Principal piece |
| PubMed | 10/20 | 4 篇 |
| PPI | 15/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
