---
type: sperm-protein-evaluation
gene: "SPINK2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# SPINK2 — 精子模块评估

## 1. 基本信息
- **基因:** SPINK2
- **Ensembl:** ENSG00000128040
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 57 篇
- **精子定位部位:** Acrosome、Equatorial segment (2 个)

## 2. HPA 精子定位证据
- **来源:** Acrosome、Equatorial segment ✓
- **链接:** https://www.proteinatlas.org/ENSG00000128040-SPINK2
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/26813/2218_D7_31_blue_red_green.jpg)
![](https://images.proteinatlas.org/26813/2218_D7_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: D6RI10 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Kazal_dom, Kazal_dom_sf, SPINK2。
Pfam: Kazal_1。

## 4. PubMed 文献证据
- **文献数:** 57 篇 (中等研究量)
- 1. PMID 31146259: The sperm-associated antigen 6 interactome and its role in spermatogenesis. (2019 Aug) *Reproduction*
2. PMID 36462395: Characterization of SPINK2, SPACA7 and PDCL2: Effect of immunization on fecundity, sperm function and testicular transcriptome. (2023 Mar) *Reprod Biol*
3. PMID 28554943: SPINK2 deficiency causes infertility by inducing sperm defects in heterozygotes and azoospermia in homozygotes. (2017 Aug) *EMBO Mol Med*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: D6RI10)。参见 https://alphafold.ebi.ac.uk/entry/D6RI10
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| SPINK2 | 0.899 | 0.000 | 0.000 | 0.000 |
| NUDT8 | 0.816 | 0.000 | 0.000 | 0.000 |
| SPINK6 | 0.723 | 0.000 | 0.000 | 0.000 |
| SPINK2 | 0.711 | 0.000 | 0.000 | 0.000 |
| SPINK2 | 0.652 | 0.000 | 0.000 | 0.000 |
| SPINK2 | 0.629 | 0.000 | 0.000 | 0.000 |
| SPINK2 | 0.606 | 0.000 | 0.000 | 0.000 |
| SPINK6 | 0.597 | 0.000 | 0.000 | 0.000 |
| SPINK2 | 0.576 | 0.000 | 0.000 | 0.000 |
| SPINK13 | 0.541 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Acrosome、Equatorial segment |
| PubMed | 7/20 | 57 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome、Equatorial segment
- 建议验证精子 IF 文献定位
