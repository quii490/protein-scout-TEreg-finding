---
type: sperm-protein-evaluation
gene: "CATSPERB"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CATSPERB — 精子模块评估

## 1. 基本信息
- **基因:** CATSPERB
- **Ensembl:** ENSG00000133962
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 15 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** GO: CatSper complex

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000133962-CATSPERB
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/35134/2239_F11_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/35134/2239_F11_8_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H0YJA5 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: CATSPERB, CATSPERB_2nd。
Pfam: CATSPERB_2nd。

## 4. PubMed 文献证据
- **文献数:** 15 篇 (低研究量)
- 1. PMID 36672807: Identification of Circular RNAs of Testis and Caput Epididymis and Prediction of Their Potential Functional Roles in Donkeys. (2022 Dec 25) *Genes (Basel)*
2. PMID 30936599: Inhibition of CatSper and Hv1 Channels and NOX5 Enzyme Affect Progesterone-Induced Increase of Intracellular Calcium Concentration and ROS Generation in Human Sperm. (2019 Mar) *Iran J Med Sci*
3. PMID 39210919: Prochloraz induced alterations in the expression of mRNA in the reproductive system of male offspring mice. (2024) *PeerJ*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H0YJA5)。参见 https://alphafold.ebi.ac.uk/entry/H0YJA5
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| CATSPER1 | 0.999 | 0.000 | 0.000 | 0.000 |
| CATSPERG | 0.998 | 0.000 | 0.000 | 0.000 |
| CATSPERD | 0.996 | 0.000 | 0.000 | 0.000 |
| CATSPER4 | 0.992 | 0.000 | 0.000 | 0.000 |
| CATSPERG | 0.992 | 0.000 | 0.000 | 0.000 |
| CATSPERZ | 0.992 | 0.000 | 0.000 | 0.000 |
| CATSPER3 | 0.989 | 0.000 | 0.000 | 0.000 |
| CATSPERD | 0.989 | 0.000 | 0.000 | 0.000 |
| EFCAB9 | 0.988 | 0.000 | 0.000 | 0.000 |
| CATSPERG | 0.988 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 8/20 | 15 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
