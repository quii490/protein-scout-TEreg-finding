---
type: sperm-protein-evaluation
gene: "ROPN1L"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# ROPN1L — 精子模块评估

## 1. 基本信息
- **基因:** ROPN1L
- **蛋白名称:** Ropporin-1-like protein
- **Ensembl:** ENSG00000145491
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 37 篇
- **精子定位部位:** End piece、Mid piece、Principal piece (3 个)
- **UniProt Subcellular Location:** Cell projection, cilium, flagellum; Cell projection, cilium

## 2. HPA 精子定位证据
- **来源:** End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000145491-ROPN1L
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/41830/2148_A5_63_blue_red_green.jpg)
![](https://images.proteinatlas.org/41830/2210_D3_98_blue_red_green.jpg)
![](https://images.proteinatlas.org/41830/2210_D3_56_blue_red_green.jpg)
![](https://images.proteinatlas.org/41830/2153_B6_48_blue_red_green.jpg)
![](https://images.proteinatlas.org/41830/2153_B6_13_blue_red_green.jpg)
![](https://images.proteinatlas.org/41830/2211_D7_6_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q96C74 — Ropporin-1-like protein.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: ROP_DD。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 37 篇 (中等研究量)
- 1. PMID 35207567: Omics and Male Infertility: Highlighting the Application of Transcriptomic Data. (2022 Feb 14) *Life (Basel)*
2. PMID 38027210: RNA sequencing profiles reveals progressively reduced spermatogenesis with progression in adult cryptorchidism. (2023) *Front Endocrinol (Lausanne)*
3. PMID 25124046: Opisthorchis viverrini: analysis of the sperm-specific rhophilin associated tail protein 1-like. (2014 Dec) *Acta Trop*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q96C74)。参见 https://alphafold.ebi.ac.uk/entry/Q96C74
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| SPA17 | 0.975 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.963 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.956 | 0.000 | 0.000 | 0.000 |
| SPA17 | 0.953 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.950 | 0.000 | 0.000 | 0.000 |
| ROPN1L | 0.942 | 0.000 | 0.000 | 0.000 |
| CABYR | 0.916 | 0.000 | 0.000 | 0.000 |
| ROPN1L | 0.860 | 0.000 | 0.000 | 0.000 |
| RHPN1 | 0.825 | 0.000 | 0.000 | 0.000 |
| ROPN1L | 0.785 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: End piece、Mid piece、Principal piece |
| PubMed | 7/20 | 37 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **68/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
