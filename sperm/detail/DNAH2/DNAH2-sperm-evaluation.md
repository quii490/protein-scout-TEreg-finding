---
type: sperm-protein-evaluation
gene: "DNAH2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DNAH2 — 精子模块评估

## 1. 基本信息
- **基因:** DNAH2
- **Ensembl:** ENSG00000183914
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 40 篇
- **精子定位部位:** End piece、Flagellar centriole (2 个)
- **UniProt Subcellular Location:** GO: dynein complex

## 2. HPA 精子定位证据
- **来源:** End piece、Flagellar centriole ✓
- **链接:** https://www.proteinatlas.org/ENSG00000183914-DNAH2
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/67103/1335_G9_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/67103/2146_D11_63_blue_red_green.jpg)
![](https://images.proteinatlas.org/67103/2175_G12_28_blue_red_green.jpg)
![](https://images.proteinatlas.org/67103/2160_A4_12_blue_red_green.jpg)
![](https://images.proteinatlas.org/67103/2238_D7_50_blue_red_green.jpg)
![](https://images.proteinatlas.org/67103/1519_H7_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: I3L4H9 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: DHC, DYH2-5-8_CC。
Pfam: DYH2-5-8_CC。

## 4. PubMed 文献证据
- **文献数:** 40 篇 (中等研究量)
- 1. PMID 39417902: Genetic etiological spectrum of sperm morphological abnormalities. (2024 Nov) *J Assist Reprod Genet*
2. PMID 40592014: Patients with mutations in DNAH2, DNAH6 and DNAH10 causing multiple morphological abnormalities of human sperm flagella achieve good ICSI outcomes. (2025 Aug) *Reprod Biomed Online*
3. PMID 33968937: Genetic Defects in DNAH2 Underlie Male Infertility With Multiple Morphological Abnormalities of the Sperm Flagella in Humans and Mice. (2021) *Front Cell Dev Biol*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: I3L4H9)。参见 https://alphafold.ebi.ac.uk/entry/I3L4H9
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DNAH2 | 0.960 | 0.000 | 0.000 | 0.000 |
| DNAH1 | 0.958 | 0.000 | 0.000 | 0.000 |
| DNAH2 | 0.957 | 0.000 | 0.000 | 0.000 |
| DNAH17 | 0.956 | 0.000 | 0.000 | 0.000 |
| DNAH17 | 0.951 | 0.000 | 0.000 | 0.000 |
| WDR78 | 0.948 | 0.000 | 0.000 | 0.000 |
| DNAH8 | 0.937 | 0.000 | 0.000 | 0.000 |
| DNAH2 | 0.929 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.928 | 0.000 | 0.000 | 0.000 |
| DNAH7 | 0.924 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ESR1 | affinity chromatography technology | 21182205 |
| COPS5 | tandem affinity purification | 21145461 |
| H2BC21 | cross-linking study | 30021884 |
| BAG2 | anti tag coimmunoprecipitation | 33961781 |
| APG16L | anti tag coimmunoprecipitation | 31015422 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: End piece、Flagellar centriole |
| PubMed | 7/20 | 40 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Flagellar centriole
- 建议验证精子 IF 文献定位
