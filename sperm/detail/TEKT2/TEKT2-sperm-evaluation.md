---
type: sperm-protein-evaluation
gene: "TEKT2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# TEKT2 — 精子模块评估

## 1. 基本信息
- **基因:** TEKT2
- **Ensembl:** ENSG00000092850
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 21 篇
- **精子定位部位:** Connecting piece、Flagellar centriole、Mid piece、Principal piece (4 个)
- **UniProt Subcellular Location:** GO: axonemal A tubule inner sheath; axonemal microtubule; centriole; microtubule cytoskeleton; microtubule organizing center

## 2. HPA 精子定位证据
- **来源:** Connecting piece、Flagellar centriole、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000092850-TEKT2
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/27461/2239_E3_43_blue_red_green.jpg)
![](https://images.proteinatlas.org/27461/2239_E3_29_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8S6V9 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 21 篇 (低研究量)
- 1. PMID 34591790: Downregulation of KIF2C and TEKT2 is associated with male infertility and testicular carcinoma. (2021 Sep 30) *Aging (Albany NY)*
2. PMID 31463202: Influence of cryopreservation on the CATSPER2 and TEKT2 expression levels and protein levels in human spermatozoa. (2019) *Toxicol Rep*
3. PMID 37891374: Identification of hub genes associated with spermatogenesis by bioinformatics analysis. (2023 Oct 27) *Sci Rep*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8S6V9)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8S6V9
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| EFHC1 | 0.966 | 0.000 | 0.000 | 0.000 |
| GSC | 0.954 | 0.000 | 0.000 | 0.000 |
| EFHC2 | 0.943 | 0.000 | 0.000 | 0.000 |
| TEKT1 | 0.923 | 0.000 | 0.000 | 0.000 |
| EFHC2 | 0.913 | 0.000 | 0.000 | 0.000 |
| TEKT1 | 0.912 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.912 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.909 | 0.000 | 0.000 | 0.000 |
| EFHC2 | 0.909 | 0.000 | 0.000 | 0.000 |
| CFAP45 | 0.905 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| EXOC1 | two hybrid fragment pooling approach | 31413325 |
| TRAF3IP1 | two hybrid fragment pooling approach | 31413325 |
| MRPL39 | inference by socio-affinity scoring |  |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 4 部位: Connecting piece、Flagellar centriole、Mid piece、Principal piece |
| PubMed | 8/20 | 21 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Connecting piece、Flagellar centriole、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
