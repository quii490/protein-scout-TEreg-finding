---
type: sperm-protein-evaluation
gene: "PTPN23"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# PTPN23 — 精子模块评估

## 1. 基本信息
- **基因:** PTPN23
- **Ensembl:** ENSG00000076201
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 75 篇
- **精子定位部位:** Calyx、Mid piece (2 个)
- **UniProt Subcellular Location:** GO: ciliary basal body; cytosol; nuclear body; nucleoplasm

## 2. HPA 精子定位证据
- **来源:** Calyx、Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000076201-PTPN23
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/16845/139_E4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/16845/166_E4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/16845/2209_B6_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/16845/138_E4_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/16845/2150_C5_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/16845/2150_C5_12_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: C9JD91 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: BRO1_dom, BRO1_sf。
Pfam: BRO1。

## 4. PubMed 文献证据
- **文献数:** 75 篇 (较多研究量)
- 1. PMID 23843459: Tumor-suppressive function of protein-tyrosine phosphatase non-receptor type 23 in testicular germ cell tumors is lost upon overexpression of miR142-3p microRNA. (2013 Aug 16) *J Biol Chem*
2. PMID 19001729: Screening of LRRK2 interactants by yeast 2-hybrid analysis. (2008 Oct) *Zhong Nan Da Xue Xue Bao Yi Xue Ban*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: C9JD91)。参见 https://alphafold.ebi.ac.uk/entry/C9JD91
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| UBAP1 | 0.999 | 0.000 | 0.000 | 0.000 |
| HGS | 0.999 | 0.000 | 0.000 | 0.000 |
| HGS | 0.999 | 0.000 | 0.000 | 0.000 |
| UBAP1 | 0.996 | 0.000 | 0.000 | 0.000 |
| PTPN23 | 0.994 | 0.000 | 0.000 | 0.000 |
| PTPN23 | 0.993 | 0.000 | 0.000 | 0.000 |
| CHMP4A | 0.991 | 0.000 | 0.000 | 0.000 |
| CHMP4A | 0.984 | 0.000 | 0.000 | 0.000 |
| STAM2 | 0.982 | 0.000 | 0.000 | 0.000 |
| UBAP1 | 0.971 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| DDX24 | two hybrid pooling approach | 16169070 |
| VCAM1 | cross-linking study | 22623428 |
| VCAM1 | cross-linking study | 22623428 |
| VCAM1 | cross-linking study | 22623428 |
| GRB2 | two hybrid pooling approach | 20936779 |
| ESR2 | cosedimentation through density gradient | 21182203 |
| EEA1 | proximity-dependent biotin identification | 29568061 |
| TGOLN2 | proximity-dependent biotin identification | 29568061 |
| NOTCH2NLA | two hybrid array | 31515488 |
| CEP19 | two hybrid array | 32296183 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Calyx、Mid piece |
| PubMed | 6/20 | 75 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Calyx、Mid piece
- 建议验证精子 IF 文献定位
