---
type: sperm-protein-evaluation
gene: "DNAH12"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DNAH12 — 精子模块评估

## 1. 基本信息
- **基因:** DNAH12
- **蛋白名称:** Dynein axonemal heavy chain 12
- **Ensembl:** ENSG00000174844
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 21 篇
- **精子定位部位:** Mid piece、Principal piece (2 个)
- **UniProt Subcellular Location:** Cell projection, cilium, flagellum; Cytoplasm, cytoskeleton, cilium axoneme

## 2. HPA 精子定位证据
- **来源:** Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000174844-DNAH12
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/37493/2032_F11_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/37493/2199_C11_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/37493/2152_G7_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/37493/2132_F7_42_blue_red_green.jpg)
![](https://images.proteinatlas.org/37493/2199_C11_16_blue_red_green.jpg)
![](https://images.proteinatlas.org/37493/2152_G7_12_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: E9PG32 — Dynein axonemal heavy chain 12.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: AAA+_ATPase, AAA_6, AAA_9, AAA_lid_11, AAA_lid_11_sf。
Pfam: AAA_6, AAA_7, AAA_8, AAA_9, AAA_lid_1。

## 4. PubMed 文献证据
- **文献数:** 21 篇 (低研究量)
- 1. PMID 39267058: Whole exome sequencing analysis of 167 men with primary infertility. (2024 Sep 12) *BMC Med Genomics*
2. PMID 40146200: Deficiency in DNAH12 causes male infertility by impairing DNAH1 and DNALI1 recruitment in humans and mice. (2025 Mar 27) *Elife*
3. PMID 39849482: IQUB mutation induces radial spoke 1 deficiency causing asthenozoospermia with normal sperm morphology in humans and mice. (2025 Jan 23) *Cell Commun Signal*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: E9PG32)。参见 https://alphafold.ebi.ac.uk/entry/E9PG32
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DNAI2 | 0.997 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.995 | 0.000 | 0.000 | 0.000 |
| DNAH6 | 0.981 | 0.000 | 0.000 | 0.000 |
| DNAH1 | 0.980 | 0.000 | 0.000 | 0.000 |
| CCDC114 | 0.965 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.963 | 0.000 | 0.000 | 0.000 |
| DNAH1 | 0.957 | 0.000 | 0.000 | 0.000 |
| DNAH12 | 0.947 | 0.000 | 0.000 | 0.000 |
| DNAH14 | 0.946 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.942 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| MAK | pull down | 32707033 |
| COQ8B | pull down | 32707033 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Mid piece、Principal piece |
| PubMed | 8/20 | 21 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **68/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece、Principal piece
- 建议验证精子 IF 文献定位
