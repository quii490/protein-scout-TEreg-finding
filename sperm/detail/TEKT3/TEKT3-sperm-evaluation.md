---
type: sperm-protein-evaluation
gene: "TEKT3"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# TEKT3 — 精子模块评估

## 1. 基本信息
- **基因:** TEKT3
- **Ensembl:** ENSG00000125409
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 27 篇
- **精子定位部位:** Calyx、End piece、Equatorial segment、Mid piece、Principal piece (5 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton, cilium axoneme; Cytoplasm, cytoskeleton, flagellum axoneme

## 2. HPA 精子定位证据
- **来源:** Calyx、End piece、Equatorial segment、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000125409-TEKT3
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/73565/2205_G11_11_blue_red_green.jpg)
![](https://images.proteinatlas.org/73565/2205_G11_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: F5H115 — Tektin.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Tektins。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 27 篇 (低研究量)
- 1. PMID 40649876: Age-Associated Proteomic Changes in Human Spermatozoa. (2025 Jun 25) *Int J Mol Sci*
2. PMID 36708031: Bi-allelic human TEKT3 mutations cause male infertility with oligoasthenoteratozoospermia owing to acrosomal hypoplasia and reduced progressive motility. (2023 May 5) *Hum Mol Genet*
3. PMID 38799722: Sperm long non-coding RNAs as markers for ram fertility. (2024) *Front Vet Sci*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: F5H115)。参见 https://alphafold.ebi.ac.uk/entry/F5H115
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| C9orf116 | 0.942 | 0.000 | 0.000 | 0.000 |
| ENKUR | 0.922 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.921 | 0.000 | 0.000 | 0.000 |
| TEKT3 | 0.913 | 0.000 | 0.000 | 0.000 |
| TEKT1 | 0.912 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.909 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.909 | 0.000 | 0.000 | 0.000 |
| RIBC2 | 0.905 | 0.000 | 0.000 | 0.000 |
| TEKT3 | 0.900 | 0.000 | 0.000 | 0.000 |
| EFHC1 | 0.900 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 5 部位: Calyx、End piece、Equatorial segment、Mid piece、Principal piece |
| PubMed | 8/20 | 27 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Calyx、End piece、Equatorial segment、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
