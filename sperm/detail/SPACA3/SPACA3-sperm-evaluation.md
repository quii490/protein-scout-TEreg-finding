---
type: sperm-protein-evaluation
gene: "SPACA3"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# SPACA3 — 精子模块评估

## 1. 基本信息
- **基因:** SPACA3
- **蛋白名称:** Sperm acrosome membrane-associated protein 3
- **Ensembl:** 
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 33 篇
- **精子定位部位:** Acrosome (1 个)
- **UniProt Subcellular Location:** Cytoplasmic vesicle, secretory vesicle, acrosome membrane

## 2. HPA 精子定位证据
- **来源:** Acrosome ✓
- **链接:** https://www.proteinatlas.org/-SPACA3
- **IF 图像:** 未获取

*HPA IF 图像未获取。已查询 HPA subcellular 页面。*


## 3. UniProt / GO-CC 精子定位证据
**UniProt Subcellular Location:** Cytoplasmic vesicle, secretory vesicle, acrosome membrane

*GO-CC 精子相关条目待进一步查询。*

## 4. PubMed 文献证据
- **文献数:** 33 篇 (中等研究量)
- 1. PMID 26748928: Acrosome markers of human sperm. (2016 Mar) *Anat Sci Int*
2. PMID 37230456: Effects of cryopreservation in the presence of Natural Deep Eutectic Solvents (NADESs) on sperm parameters. (2023 Sep) *Cryobiology*
3. PMID 37657147: Comparing ovarian expression of sperm acrosome associated 3 protein in young and adult queens. (2023 Nov) *Theriogenology*

## 5. AlphaFold / PAE / PDB / 结构域
*待结构数据采集。*
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| SPACA1 | 0.835 | 0.000 | 0.000 | 0.000 |
| IZUMO1 | 0.794 | 0.000 | 0.000 | 0.000 |
| SPACA3 | 0.706 | 0.000 | 0.000 | 0.000 |
| SPESP1 | 0.681 | 0.000 | 0.000 | 0.000 |
| SPESP1 | 0.664 | 0.000 | 0.000 | 0.000 |
| TMEM190 | 0.657 | 0.000 | 0.000 | 0.000 |
| ACRBP | 0.649 | 0.000 | 0.000 | 0.000 |
| SPESP1 | 0.591 | 0.000 | 0.000 | 0.000 |
| IZUMO1 | 0.583 | 0.000 | 0.000 | 0.000 |
| IZUMO1 | 0.581 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ZP3 | two hybrid | 19945174 |
| ZP3 | gal4 vp16 complementation | 19945174 |
| GDAP1 | two hybrid array | 32814053 |
| GDAP1 | validated two hybrid | 32814053 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Acrosome |
| PubMed | 7/20 | 33 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | 待采集 |
| 新颖性 | 6/10 | 中等 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome
- 建议验证精子 IF 文献定位
