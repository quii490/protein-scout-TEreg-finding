---
type: sperm-protein-evaluation
gene: "HAUS7"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# HAUS7 — 精子模块评估

## 1. 基本信息
- **基因:** HAUS7
- **Ensembl:** ENSG00000213397
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 9 篇
- **精子定位部位:** Mid piece、Principal piece (2 个)
- **UniProt Subcellular Location:** GO: centrosome

## 2. HPA 精子定位证据
- **来源:** Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000213397-HAUS7
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/3614/57_B7_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/3614/2120_D2_48_blue_red_green.jpg)
![](https://images.proteinatlas.org/3614/58_B7_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/3614/2209_A8_26_blue_red_green.jpg)
![](https://images.proteinatlas.org/3614/56_B7_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/3614/2209_A8_11_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H7BZF8 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Haus7-like。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 9 篇 (极低研究量)
- 1. PMID 31589365: The dynamics of gene expression during and post meiosis sets the sperm agenda. (2019 Dec) *Mol Reprod Dev*
2. PMID 34552771: Current updates and future perspectives in the evaluation of azoospermia: A systematic review. (2021) *Arab J Urol*
3. PMID 29017965: A novel mutation in HAUS7 results in severe oligozoospermia in two brothers. (2018 Jan 10) *Gene*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H7BZF8)。参见 https://alphafold.ebi.ac.uk/entry/H7BZF8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| HAUS6 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS5 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS1 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS3 | 0.999 | 0.000 | 0.000 | 0.000 |
| UCHL5 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS1 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS5 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS6 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS8 | 0.999 | 0.000 | 0.000 | 0.000 |
| HAUS2 | 0.999 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| CREB2 | two hybrid pooling approach | 16189514 |
| MED29 | anti tag coimmunoprecipitation | 15175163 |
| CREB2 | two hybrid pooling approach | 19447967 |
| HAUS2 | tandem affinity purification | 20360068 |
| HAUS6 | tandem affinity purification | 20360068 |
| HAUS4 | tandem affinity purification | 20360068 |
| HAUS1 | tandem affinity purification | 20360068 |
| ACOX3 | anti tag coimmunoprecipitation | 28514442 |
| HSCB | anti bait coimmunoprecipitation | 28380382 |
| DPPA3 | validated two hybrid | 32296183 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Mid piece、Principal piece |
| PubMed | 10/20 | 9 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece、Principal piece
- 建议验证精子 IF 文献定位
