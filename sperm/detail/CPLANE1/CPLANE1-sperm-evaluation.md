---
type: sperm-protein-evaluation
gene: "CPLANE1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CPLANE1 — 精子模块评估

## 1. 基本信息
- **基因:** CPLANE1
- **Ensembl:** ENSG00000197603
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 66 篇
- **精子定位部位:** Mid piece、Principal piece (2 个)
- **UniProt Subcellular Location:** GO: ciliary transition zone; membrane

## 2. HPA 精子定位证据
- **来源:** Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000197603-CPLANE1
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/36776/596_B7_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/36776/2153_F8_48_blue_red_green.jpg)
![](https://images.proteinatlas.org/36776/2168_A12_56_blue_red_green.jpg)
![](https://images.proteinatlas.org/36776/593_B7_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/36776/2153_F8_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/36776/2145_C7_42_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H0Y9I8 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: CPLANE1。
Pfam: Joubert。

## 4. PubMed 文献证据
- **文献数:** 66 篇 (较多研究量)
- 1. PMID 33573944: Genome-wide association studies for sperm traits in Assaf sheep breed. (2021 Feb) *Animal*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H0Y9I8)。参见 https://alphafold.ebi.ac.uk/entry/H0Y9I8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| TMEM67 | 0.998 | 0.000 | 0.000 | 0.000 |
| CC2D2A | 0.998 | 0.000 | 0.000 | 0.000 |
| CC2D2A | 0.997 | 0.000 | 0.000 | 0.000 |
| CC2D2A | 0.994 | 0.000 | 0.000 | 0.000 |
| TMEM67 | 0.993 | 0.000 | 0.000 | 0.000 |
| TCTN3 | 0.993 | 0.000 | 0.000 | 0.000 |
| TCTN3 | 0.988 | 0.000 | 0.000 | 0.000 |
| TMEM216 | 0.987 | 0.000 | 0.000 | 0.000 |
| TCTN3 | 0.984 | 0.000 | 0.000 | 0.000 |
| WDR70 | 0.944 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| C5orf42 | two hybrid pooling approach | 20936779 |
| KIAA0829 | tandem affinity purification | 21145461 |
| AUH | cross-linking study | 30021884 |
| C5orf42 | cross-linking study | 30021884 |
| C5orf42 | cross-linking study | 30021884 |
| DISC1 | two hybrid fragment pooling approach | 31413325 |
| BRAP | two hybrid | 25820252 |
| TUBGCP4 | anti tag coimmunoprecipitation | 26496610 |
| S100B | anti tag coimmunoprecipitation | 33961781 |
| TESK2 | pull down | 32707033 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Mid piece、Principal piece |
| PubMed | 6/20 | 66 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece、Principal piece
- 建议验证精子 IF 文献定位
