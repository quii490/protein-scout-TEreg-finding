---
type: sperm-protein-evaluation
gene: "DNAH17"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DNAH17 — 精子模块评估

## 1. 基本信息
- **基因:** DNAH17
- **Ensembl:** ENSG00000187775
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 73 篇
- **精子定位部位:** End piece、Mid piece、Principal piece (3 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton, cilium axoneme

## 2. HPA 精子定位证据
- **来源:** End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000187775-DNAH17
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/24354/2229_G5_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/24354/2229_G5_11_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: K7ELN3 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: AAA_9, AAA_lid_11, AAA_lid_11_sf, DHC, Dynein_C。
Pfam: AAA_8, AAA_9, AAA_lid_11, Dynein_C, Dynein_heavy。

## 4. PubMed 文献证据
- **文献数:** 73 篇 (较多研究量)
- 1. PMID 39417902: Genetic etiological spectrum of sperm morphological abnormalities. (2024 Nov) *J Assist Reprod Genet*
2. PMID 37574497: Novel mutations in DNAH17 cause sperm flagellum defects and their influence on ICSI outcome. (2023 Oct) *J Assist Reprod Genet*
3. PMID 39267058: Whole exome sequencing analysis of 167 men with primary infertility. (2024 Sep 12) *BMC Med Genomics*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: K7ELN3)。参见 https://alphafold.ebi.ac.uk/entry/K7ELN3
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| *STRING 无数据* | — | — | — | — |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| CXCR2 | anti bait coimmunoprecipitation | 21876773 |
| IRS4 | anti tag coimmunoprecipitation | 25036637 |
| BMI1 | anti bait coimmunoprecipitation | 34316702 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: End piece、Mid piece、Principal piece |
| PubMed | 6/20 | 73 篇 |
| PPI | 5/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **50/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
