---
type: sperm-protein-evaluation
gene: "NDUFA6"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# NDUFA6 — 精子模块评估

## 1. 基本信息
- **基因:** NDUFA6
- **蛋白名称:** NADH dehydrogenase [ubiquinone] 1 alpha subcomplex subunit 6
- **Ensembl:** ENSG00000184983
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 58 篇
- **精子定位部位:** End piece、Mid piece (2 个)
- **UniProt Subcellular Location:** GO: respiratory chain complex I

## 2. HPA 精子定位证据
- **来源:** End piece、Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000184983-NDUFA6
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/56934/2212_E11_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/56934/963_H8_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/56934/1044_G10_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/56934/968_H8_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/56934/2212_E11_11_blue_red_green.jpg)
![](https://images.proteinatlas.org/56934/963_H8_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A2Y9D025 — NADH dehydrogenase [ubiquinone] 1 alpha subcomplex subunit 6.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Complex1_LYR_NDUFA6_LYRM6, NADH_Ub_cplx-1_asu_su-6。
Pfam: Complex1_LYR_2。

## 4. PubMed 文献证据
- **文献数:** 58 篇 (中等研究量)
- 1. PMID 34330296: iTRAQ-based proteomic analysis of sperm reveals candidate proteins that affect the quality of spermatozoa from boars on plateaus. (2021 Jul 30) *Proteome Sci*
2. PMID 36108438: Benzo[a]pyrene inhibits testosterone biosynthesis via NDUFA10-mediated mitochondrial compromise in mouse Leydig cells: Integrating experimental and in silico toxicological approaches. (2022 Oct 1) *Ecotoxicol Environ Saf*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A2Y9D025)。参见 https://alphafold.ebi.ac.uk/entry/A0A2Y9D025
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| NDUFB5 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFA9 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFA6 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFS6 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFS3 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFB7 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFS7 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFA10 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFA2 | 0.999 | 0.000 | 0.000 | 0.000 |
| NDUFB10 | 0.999 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ATP5MF | two hybrid pooling approach | 16169070 |
| LYRM6 | two hybrid pooling approach | 16169070 |
| PHLDA3 | anti bait coimmunoprecipitation | 17353931 |
| NDUFA2 | blue native page | 19688755 |
| NDUFA2 | blue native page | 19688755 |
| HSPD1 | proximity-dependent biotin identification | 29568061 |
| COX14 | proximity-dependent biotin identification | 29568061 |
| MGST3 | proximity-dependent biotin identification | 29568061 |
| TRMT61B | proximity-dependent biotin identification | 29568061 |
| NDUFS3 | anti tag coimmunoprecipitation | 27499296 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: End piece、Mid piece |
| PubMed | 7/20 | 58 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: End piece、Mid piece
- 建议验证精子 IF 文献定位
