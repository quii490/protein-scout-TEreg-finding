---
type: sperm-protein-evaluation
gene: "ATP5F1D"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# ATP5F1D — 精子模块评估

## 1. 基本信息
- **基因:** ATP5F1D
- **Ensembl:** ENSG00000099624
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 18 篇
- **精子定位部位:** Connecting piece、Flagellar centriole、Principal piece (3 个)
- **UniProt Subcellular Location:** GO: mitochondrial inner membrane; mitochondrial matrix; mitochondrion; proton-transporting ATP synthase complex

## 2. HPA 精子定位证据
- **来源:** Connecting piece、Flagellar centriole、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000099624-ATP5F1D
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/2865/37_A11_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/2865/36_A11_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/2865/37_A11_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/2865/36_A11_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/2865/2204_G1_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/2865/2204_G1_31_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8SDV7 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 18 篇 (低研究量)
- 1. PMID 38674457: Effects of Long-Term Cryopreservation on the Transcriptomes of Giant Grouper Sperm. (2024 Apr 22) *Genes (Basel)*
2. PMID 23374786: Study of sperm protein profile in men with and without varicocele using two-dimensional gel electrophoresis. (2013 Feb) *Urology*
3. PMID 33615715: Altered mitochondrial function in spermatozoa from patients with repetitive fertilization failure after ICSI revealed by proteomics. (2021 Jul) *Andrology*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8SDV7)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8SDV7
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| *STRING 无数据* | — | — | — | — |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| RIPK2 | anti bait coimmunoprecipitation | 17353931 |
| SGK1 | anti bait coimmunoprecipitation | 17353931 |
| EIF1B | anti bait coimmunoprecipitation | 17353931 |
| PSMD6 | anti bait coimmunoprecipitation | 17353931 |
| ATG5 | anti bait coimmunoprecipitation | 17353931 |
| TTN | two hybrid fragment pooling approach | 23414517 |
| ATP5PB | blue native page | 19688755 |
| ATP5PB | blue native page | 19688755 |
| ACAD9 | blue native page | 23260140 |
| EPB41L4O | anti tag coimmunoprecipitation | 24366813 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: Connecting piece、Flagellar centriole、Principal piece |
| PubMed | 8/20 | 18 篇 |
| PPI | 5/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **56/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Connecting piece、Flagellar centriole、Principal piece
- 建议验证精子 IF 文献定位
