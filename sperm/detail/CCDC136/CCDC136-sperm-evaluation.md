---
type: sperm-protein-evaluation
gene: "CCDC136"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CCDC136 — 精子模块评估

## 1. 基本信息
- **基因:** CCDC136
- **Ensembl:** ENSG00000128596
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 13 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** GO: acrosomal membrane

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000128596-CCDC136
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/10646/2055_A6_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/10646/1979_A9_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/10646/1979_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/71015/2217_D4_21_blue_red_green.jpg)
![](https://images.proteinatlas.org/71015/2217_D4_41_blue_red_green.jpg)
![](https://images.proteinatlas.org/10646/1902_B7_4_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8PH14 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 13 篇 (低研究量)
- 1. PMID 27076447: A Novel Testis-Specific Gene, Ccdc136, Is Required for Acrosome Formation and Fertilization in Mice. (2016 Oct) *Reprod Sci*
2. PMID 40276155: Proteomic analysis of Toraya buffalo seminal plasma and sperm: uncovering insights to optimize reproductive success. (2025) *Front Vet Sci*
3. PMID 41806660: Proteomic insights into the effects of cryopreservation on sperm of endangered Eld's deer (Rucervus eldii). (2026 Jun) *Reprod Biol*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8PH14)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8PH14
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| MEGF10 | 0.832 | 0.000 | 0.000 | 0.000 |
| MEGF10 | 0.810 | 0.000 | 0.000 | 0.000 |
| TMEM8B | 0.679 | 0.000 | 0.000 | 0.000 |
| GCC2 | 0.634 | 0.000 | 0.000 | 0.000 |
| MEGF6 | 0.624 | 0.000 | 0.000 | 0.000 |
| MEGF11 | 0.619 | 0.000 | 0.000 | 0.000 |
| CCDC136 | 0.606 | 0.000 | 0.000 | 0.000 |
| AP2M1 | 0.576 | 0.000 | 0.000 | 0.000 |
| CCDC136 | 0.572 | 0.000 | 0.000 | 0.000 |
| FAM90A7P | 0.570 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ZNF408 | two hybrid | 15231747 |
| UBQLN4 | two hybrid | 16713569 |
| COIL | two hybrid | 16713569 |
| GFI1B | two hybrid | 16713569 |
| DISC1 | two hybrid fragment pooling approach | 31413325 |
| CDC5L | two hybrid fragment pooling approach | 31413325 |
| TXLNA | two hybrid array | 31515488 |
| PDLIM5 | two hybrid array | 31515488 |
| FAM107A | two hybrid array | 31515488 |
| ZNF581 | two hybrid prey pooling approach | 25416956 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 8/20 | 13 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
