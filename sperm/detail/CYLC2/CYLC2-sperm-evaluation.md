---
type: sperm-protein-evaluation
gene: "CYLC2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CYLC2 — 精子模块评估

## 1. 基本信息
- **基因:** CYLC2
- **Ensembl:** ENSG00000155833
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 15 篇
- **精子定位部位:** Calyx、Perinuclear theca (2 个)

## 2. HPA 精子定位证据
- **来源:** Calyx、Perinuclear theca ✓
- **链接:** https://www.proteinatlas.org/ENSG00000155833-CYLC2
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/62456/2228_C2_11_blue_red_green.jpg)
![](https://images.proteinatlas.org/62456/2228_C2_36_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q6PEJ5 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: CYLC, Cylicin_N。
Pfam: Cylicin_N。

## 4. PubMed 文献证据
- **文献数:** 15 篇 (低研究量)
- 1. PMID 26771610: Optimizing a Male Reproductive Aging Mouse Model by D-Galactose Injection. (2016 Jan 13) *Int J Mol Sci*
2. PMID 33285124: Comparison of spermatozoal RNA extraction methods in goats. (2021 Feb 1) *Anal Biochem*
3. PMID 31134708: Proteomic markers of low and high fertility bovine spermatozoa separated by Percoll gradient. (2019 Aug) *Mol Reprod Dev*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q6PEJ5)。参见 https://alphafold.ebi.ac.uk/entry/Q6PEJ5
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| CYLC2 | 0.989 | 0.000 | 0.000 | 0.000 |
| CCIN | 0.982 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.663 | 0.000 | 0.000 | 0.000 |
| CSNKA2IP | 0.574 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.564 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.553 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.522 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.521 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.517 | 0.000 | 0.000 | 0.000 |
| OR2T10 | 0.505 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Calyx、Perinuclear theca |
| PubMed | 8/20 | 15 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **68/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Calyx、Perinuclear theca
- 建议验证精子 IF 文献定位
