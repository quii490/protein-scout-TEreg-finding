---
type: sperm-protein-evaluation
gene: "ENSG00000285827"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# ENSG00000285827 — 精子模块评估

## 1. 基本信息
- **基因:** ENSG00000285827
- **Ensembl:** ENSG00000285827
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 0 篇
- **精子定位部位:** Principal piece (1 个)

## 2. HPA 精子定位证据
- **来源:** Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000285827-ENSG00000285827
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/44629/884_C1_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/44629/2212_H5_26_blue_red_green.jpg)
![](https://images.proteinatlas.org/44629/978_C1_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/44629/2212_H5_31_blue_red_green.jpg)
![](https://images.proteinatlas.org/44629/884_C1_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/44629/824_C1_3_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
*待 UniProt/GO-CC 采集。*

## 4. PubMed 文献证据
- **文献数:** 0 篇 (极低研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
*待结构数据采集。*
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| ATP5MG | 0.999 | 0.000 | 0.000 | 0.000 |
| DPY30 | 0.999 | 0.000 | 0.000 | 0.000 |
| ASH2L | 0.999 | 0.000 | 0.000 | 0.000 |
| HCFC1 | 0.999 | 0.000 | 0.000 | 0.000 |
| ASH2L | 0.999 | 0.000 | 0.000 | 0.000 |
| DPY30 | 0.999 | 0.000 | 0.000 | 0.000 |
| DPY30 | 0.997 | 0.000 | 0.000 | 0.000 |
| MEN1 | 0.992 | 0.000 | 0.000 | 0.000 |
| ASH2L | 0.987 | 0.000 | 0.000 | 0.000 |
| HCFC1 | 0.982 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Principal piece |
| PubMed | 10/20 | 0 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | 待采集 |
| 新颖性 | 10/10 | 极低 |

- **评分:** **69/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Principal piece
- 建议验证精子 IF 文献定位
