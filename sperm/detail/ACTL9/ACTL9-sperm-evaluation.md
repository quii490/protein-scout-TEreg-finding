---
type: sperm-protein-evaluation
gene: "ACTL9"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# ACTL9 — 精子模块评估

## 1. 基本信息
- **基因:** ACTL9
- **Ensembl:** ENSG00000181786
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 25 篇
- **精子定位部位:** Acrosome、Equatorial segment (2 个)

## 2. HPA 精子定位证据
- **来源:** Acrosome、Equatorial segment ✓
- **链接:** https://www.proteinatlas.org/ENSG00000181786-ACTL9
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/26693/2195_E8_16_blue_red_green.jpg)
![](https://images.proteinatlas.org/26693/2195_E8_26_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
*待 UniProt/GO-CC 采集。*

## 4. PubMed 文献证据
- **文献数:** 25 篇 (低研究量)
- 1. PMID 37212058: Exonic genetic variants associated with unexpected fertilization failure and zygotic arrest after ICSI: a systematic review. (2023 Aug) *Zygote*
2. PMID 36306217: Testis-specific serine kinase 3 is required for sperm morphogenesis and male fertility. (2023 Jul) *Andrology*
3. PMID 38414365: Deletion of ACTRT1 is associated with male infertility as sperm acrosomal ultrastructural defects and fertilization failure in human. (2024 May 2) *Hum Reprod*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q8TC94)。参见 https://alphafold.ebi.ac.uk/entry/Q8TC94
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| ACTR10 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN2 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN5 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN4 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN3 | 0.999 | 0.000 | 0.000 | 0.000 |
| ACTR1A | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN2 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN5 | 0.999 | 0.000 | 0.000 | 0.000 |
| DCTN4 | 0.999 | 0.000 | 0.000 | 0.000 |
| ACTR1A | 0.999 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Acrosome、Equatorial segment |
| PubMed | 8/20 | 25 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **68/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome、Equatorial segment
- 建议验证精子 IF 文献定位
