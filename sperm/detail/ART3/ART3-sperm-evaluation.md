---
type: sperm-protein-evaluation
gene: "ART3"
module: sperm
status: sperm_low_priority
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# ART3 — 精子模块评估

## 1. 基本信息
- **基因:** ART3
- **蛋白名称:** NAD(P)(+)--arginine ADP-ribosyltransferase
- **Ensembl:** ENSG00000156219
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 63 篇
- **精子定位部位:** End piece、Principal piece (2 个)
- **UniProt Subcellular Location:** Cell membrane

## 2. HPA 精子定位证据
- **来源:** End piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000156219-ART3
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/11268/2229_D7_16_blue_red_green.jpg)
![](https://images.proteinatlas.org/11268/2229_D7_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A8V8TPP7 — NAD(P)(+)--arginine ADP-ribosyltransferase.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: ADP-ribosyltransferase_ARG, ART。
Pfam: ART。

## 4. PubMed 文献证据
- **文献数:** 63 篇 (较多研究量)
- 1. PMID 18266473: Genome-wide expression of azoospermia testes demonstrates a specific profile and implicates ART3 in genetic susceptibility. (2008 Feb) *PLoS Genet*
2. PMID 16934346: Genomic organization and expression of the human mono-ADP-ribosyltransferase ART3 gene. (2006 Jun) *Biochim Biophys Acta*
3. PMID 9119374: Two novel human members of an emerging mammalian gene family related to mono-ADP-ribosylating bacterial toxins. (1997 Feb 1) *Genomics*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A8V8TPP7)。参见 https://alphafold.ebi.ac.uk/entry/A0A8V8TPP7
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| *STRING 无数据* | — | — | — | — |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: End piece、Principal piece |
| PubMed | 6/20 | 63 篇 |
| PPI | 5/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **47/100**

## 8. 结论
**SPERM LOW PRIORITY**

## 9. 人工复核备注
- 精子部位: End piece、Principal piece
- 建议验证精子 IF 文献定位
