---
type: sperm-protein-evaluation
gene: "CBLN2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CBLN2 — 精子模块评估

## 1. 基本信息
- **基因:** CBLN2
- **Ensembl:** ENSG00000141668
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 69 篇
- **精子定位部位:** Calyx、Flagellar centriole、Mid piece、Perinuclear theca、Principal piece (5 个)

## 2. HPA 精子定位证据
- **来源:** Calyx、Flagellar centriole、Mid piece、Perinuclear theca、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000141668-CBLN2
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/56335/1820_F9_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/56335/1681_E3_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/56335/1820_F9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/56335/1681_E3_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/56335/2200_G1_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/56335/1675_E3_32_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: J3QS23 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: C1q_dom, Cerebellin_Synaptic_Org, Tumour_necrosis_fac-like_dom。
Pfam: C1q。

## 4. PubMed 文献证据
- **文献数:** 69 篇 (较多研究量)
- 1. PMID 19082514: Precerebellin-related genes and precerebellin 1 peptide in endocrine glands of the rat - pattern of their expression. (2009 Jan) *Int J Mol Med*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: J3QS23)。参见 https://alphafold.ebi.ac.uk/entry/J3QS23
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| NLGN1 | 0.999 | 0.000 | 0.000 | 0.000 |
| NLGN3 | 0.999 | 0.000 | 0.000 | 0.000 |
| NRXN1 | 0.999 | 0.000 | 0.000 | 0.000 |
| NLGN1 | 0.999 | 0.000 | 0.000 | 0.000 |
| NRXN3 | 0.964 | 0.000 | 0.000 | 0.000 |
| NRXN3 | 0.940 | 0.000 | 0.000 | 0.000 |
| NRXN1 | 0.922 | 0.000 | 0.000 | 0.000 |
| NRXN1 | 0.915 | 0.000 | 0.000 | 0.000 |
| GRID2 | 0.880 | 0.000 | 0.000 | 0.000 |
| GRID2 | 0.878 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 5 部位: Calyx、Flagellar centriole、Mid piece、Perinuclear theca、Principal piece |
| PubMed | 6/20 | 69 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **66/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Calyx、Flagellar centriole、Mid piece、Perinuclear theca、Principal piece
- 建议验证精子 IF 文献定位
