---
type: sperm-protein-evaluation
gene: "CCDC65"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CCDC65 — 精子模块评估

## 1. 基本信息
- **基因:** CCDC65
- **蛋白名称:** Dynein regulatory complex subunit 2
- **Ensembl:** ENSG00000139537
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 20 篇
- **精子定位部位:** Annulus、Mid piece、Principal piece (3 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton, flagellum basal body; Cell projection, cilium, flagellum; Cytoplasm, cytoskeleton, flagellum axoneme

## 2. HPA 精子定位证据
- **来源:** Annulus、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000139537-CCDC65
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/57573/2199_H7_21_blue_red_green.jpg)
![](https://images.proteinatlas.org/57573/1741_E7_8_cr580600b1c50ce_blue_red_green.jpg)
![](https://images.proteinatlas.org/57573/2199_H7_11_blue_red_green.jpg)
![](https://images.proteinatlas.org/57573/1741_E7_18_cr580cfa63a1541_blue_red_green.jpg)
![](https://images.proteinatlas.org/57573/2161_A8_24_blue_red_green.jpg)
![](https://images.proteinatlas.org/57573/2245_C5_28_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q8IXS2 — Dynein regulatory complex subunit 2.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: DRC1/2_N, DRC1/DRC2。
Pfam: NYD-SP28。

## 4. PubMed 文献证据
- **文献数:** 20 篇 (低研究量)
- 1. PMID 24152136: Methylation of structural components of the axoneme occurs during flagellar disassembly. (2013 Nov 26) *Biochemistry*
2. PMID 37975235: CCDC65, encoding a component of the axonemal Nexin-Dynein regulatory complex, is required for sperm flagellum structure in humans. (2024 Mar) *Clin Genet*
3. PMID 17089017: Cloning and characterization of a novel sperm tail protein, NYD-SP28. (2006 Dec) *Int J Mol Med*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q8IXS2)。参见 https://alphafold.ebi.ac.uk/entry/Q8IXS2
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| CCDC39 | 0.995 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.972 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.963 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.960 | 0.000 | 0.000 | 0.000 |
| CCDC65 | 0.940 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.938 | 0.000 | 0.000 | 0.000 |
| CCDC65 | 0.933 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.920 | 0.000 | 0.000 | 0.000 |
| CCDC40 | 0.911 | 0.000 | 0.000 | 0.000 |
| DNAI1 | 0.906 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: Annulus、Mid piece、Principal piece |
| PubMed | 8/20 | 20 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **71/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Annulus、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
