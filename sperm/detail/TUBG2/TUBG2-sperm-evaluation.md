---
type: sperm-protein-evaluation
gene: "TUBG2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# TUBG2 — 精子模块评估

## 1. 基本信息
- **基因:** TUBG2
- **蛋白名称:** Tubulin gamma-2 chain
- **Ensembl:** ENSG00000037042
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 18 篇
- **精子定位部位:** Principal piece (1 个)
- **UniProt Subcellular Location:** Cytoplasm, cytoskeleton, microtubule organizing center, centrosome

## 2. HPA 精子定位证据
- **来源:** Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000037042-TUBG2
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/11495/636_A3_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/11495/636_A3_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/43012/2215_G5_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/11495/638_A3_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/11495/637_A3_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/11495/637_A3_2_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q9NRH3 — Tubulin gamma-2 chain.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Gamma_tubulin, Tub_FtsZ_C, Tubulin, Tubulin/FtsZ-like_C, Tubulin/FtsZ_2-layer-sand-dom。
Pfam: Tubulin, Tubulin_C。

## 4. PubMed 文献证据
- **文献数:** 18 篇 (低研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q9NRH3)。参见 https://alphafold.ebi.ac.uk/entry/Q9NRH3
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| TUBG1 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP2 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP3 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP4 | 0.999 | 0.000 | 0.000 | 0.000 |
| MZT1 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP5 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP5 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP4 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP3 | 0.999 | 0.000 | 0.000 | 0.000 |
| TUBGCP2 | 0.999 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| ? | anti bait coimmunoprecipitation | 17353931 |
| CIAO1 | anti bait coimmunoprecipitation | 17353931 |
| PTP4A3 | anti bait coimmunoprecipitation | 17353931 |
| PEX14 | pull down | 21525035 |
| PEX14 | pull down | 21525035 |
| TUBG1 | proximity-dependent biotin identification | 29568061 |
| DISC1 | two hybrid fragment pooling approach | 31413325 |
| MAPK1 | anti tag coimmunoprecipitation | 31980649 |
| RAF1 | anti tag coimmunoprecipitation | 31980649 |
| RAF1 | anti tag coimmunoprecipitation | 31980649 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Principal piece |
| PubMed | 8/20 | 18 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Principal piece
- 建议验证精子 IF 文献定位
