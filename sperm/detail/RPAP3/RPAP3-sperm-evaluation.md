---
type: sperm-protein-evaluation
gene: "RPAP3"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# RPAP3 — 精子模块评估

## 1. 基本信息
- **基因:** RPAP3
- **Ensembl:** ENSG00000005175
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 60 篇
- **精子定位部位:** Acrosome (1 个)
- **UniProt Subcellular Location:** GO: cytosol; R2TP complex

## 2. HPA 精子定位证据
- **来源:** Acrosome ✓
- **链接:** https://www.proteinatlas.org/ENSG00000005175-RPAP3
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/38311/566_E10_4_blue_red_green.jpg)
![](https://images.proteinatlas.org/38311/566_E10_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/38311/2244_B5_13_blue_red_green.jpg)
![](https://images.proteinatlas.org/38311/si22_H6_34_blue_red_green.jpg)
![](https://images.proteinatlas.org/38311/563_E10_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/38311/2241_D7_28_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8RMB8 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 60 篇 (中等研究量)
- 1. PMID 29844425: The RPAP3-Cterminal domain identifies R2TP-like quaternary chaperones. (2018 May 29) *Nat Commun*
2. PMID 30428028: Chlamydomonas WDR92 in association with R2TP-like complex and multiple DNAAFs to regulate ciliary dynein preassembly. (2019 Sep 19) *J Mol Cell Biol*
3. PMID 31858719: The outer dynein arm assembly factor CCDC103 forms molecular scaffolds through multiple self-interaction sites. (2020 Jan) *Cytoskeleton (Hoboken)*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8RMB8)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8RMB8
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| RUVBL1 | 0.999 | 0.000 | 0.000 | 0.000 |
| PIH1D1 | 0.999 | 0.000 | 0.000 | 0.000 |
| RUVBL2 | 0.999 | 0.000 | 0.000 | 0.000 |
| PFDN6 | 0.999 | 0.000 | 0.000 | 0.000 |
| UXT | 0.999 | 0.000 | 0.000 | 0.000 |
| RUVBL2 | 0.999 | 0.000 | 0.000 | 0.000 |
| RUVBL1 | 0.999 | 0.000 | 0.000 | 0.000 |
| RUVBL2 | 0.999 | 0.000 | 0.000 | 0.000 |
| PFDN6 | 0.999 | 0.000 | 0.000 | 0.000 |
| UXT | 0.998 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Acrosome |
| PubMed | 7/20 | 60 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome
- 建议验证精子 IF 文献定位
