---
type: sperm-protein-evaluation
gene: "DNAL1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# DNAL1 — 精子模块评估

## 1. 基本信息
- **基因:** DNAL1
- **Ensembl:** ENSG00000119661
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 34 篇
- **精子定位部位:** Acrosome、End piece、Mid piece、Principal piece (4 个)
- **UniProt Subcellular Location:** GO: microtubule; outer dynein arm

## 2. HPA 精子定位证据
- **来源:** Acrosome、End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000119661-DNAL1
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/53129/833_F1_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/53129/2133_D11_22_blue_red_green.jpg)
![](https://images.proteinatlas.org/53129/833_F1_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/53129/2199_B11_31_blue_red_green.jpg)
![](https://images.proteinatlas.org/53129/925_C7_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/53129/925_C7_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: G3V2M0 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: LRR_dom_sf, NXF1/2/3/5-like_LRR。
Pfam: LRR_NXF1-5。

## 4. PubMed 文献证据
- **文献数:** 34 篇 (中等研究量)
- 1. PMID 20301301: Primary Ciliary Dyskinesia. (1993) **
2. PMID 38454159: BAG5 regulates HSPA8-mediated protein folding required for sperm head-tail coupling apparatus assembly. (2024 Apr) *EMBO Rep*
3. PMID 39180133: Characterization of pathogenic genetic variants in Russian patients with primary ciliary dyskinesia using gene panel sequencing and transcript analysis. (2024 Aug 23) *Orphanet J Rare Dis*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: G3V2M0)。参见 https://alphafold.ebi.ac.uk/entry/G3V2M0
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DNAI2 | 0.997 | 0.000 | 0.000 | 0.000 |
| DNAL1 | 0.979 | 0.000 | 0.000 | 0.000 |
| DNAL1 | 0.968 | 0.000 | 0.000 | 0.000 |
| CCDC114 | 0.965 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.963 | 0.000 | 0.000 | 0.000 |
| DNAH2 | 0.960 | 0.000 | 0.000 | 0.000 |
| DNAH2 | 0.920 | 0.000 | 0.000 | 0.000 |
| MAP1LC3A | 0.914 | 0.000 | 0.000 | 0.000 |
| DNAH7 | 0.909 | 0.000 | 0.000 | 0.000 |
| DNAI2 | 0.903 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 4 部位: Acrosome、End piece、Mid piece、Principal piece |
| PubMed | 7/20 | 34 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **69/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Acrosome、End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
