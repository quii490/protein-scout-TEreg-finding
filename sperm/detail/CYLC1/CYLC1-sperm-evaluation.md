---
type: sperm-protein-evaluation
gene: "CYLC1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CYLC1 — 精子模块评估

## 1. 基本信息
- **基因:** CYLC1
- **Ensembl:** ENSG00000183035
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 8 篇
- **精子定位部位:** Calyx、Perinuclear theca (2 个)
- **UniProt Subcellular Location:** GO: acrosomal matrix; cytoskeletal calyx; nucleus; perinuclear theca; sperm glycocalyx

## 2. HPA 精子定位证据
- **来源:** Calyx、Perinuclear theca ✓
- **链接:** https://www.proteinatlas.org/ENSG00000183035-CYLC1
- **IF 图像:** 已获取 (4 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (4 张 blue_red_green)。
![](https://images.proteinatlas.org/78303/2233_B5_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/70355/2228_F3_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/78303/2233_B5_22_blue_red_green.jpg)
![](https://images.proteinatlas.org/70355/2228_F3_11_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q3ZCV3 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: CYLC。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 8 篇 (极低研究量)
- 1. PMID 38573307: Disruption in CYLC1 leads to acrosome detachment, sperm head deformity, and male in/subfertility in humans and mice. (2024 Apr 4) *Elife*
2. PMID 41075067: Novel CYLC1 variants are associated with male infertility due to sperm head deformity. (2025 Nov) *J Assist Reprod Genet*
3. PMID 36896575: The human sperm proteome-Toward a panel for male fertility testing. (2023 Oct) *Andrology*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q3ZCV3)。参见 https://alphafold.ebi.ac.uk/entry/Q3ZCV3
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| CYLC2 | 0.989 | 0.000 | 0.000 | 0.000 |
| CCIN | 0.982 | 0.000 | 0.000 | 0.000 |
| CYLC2 | 0.663 | 0.000 | 0.000 | 0.000 |
| CYLC1 | 0.652 | 0.000 | 0.000 | 0.000 |
| SATL1 | 0.613 | 0.000 | 0.000 | 0.000 |
| SATL1 | 0.583 | 0.000 | 0.000 | 0.000 |
| SATL1 | 0.583 | 0.000 | 0.000 | 0.000 |
| APOOL | 0.579 | 0.000 | 0.000 | 0.000 |
| CYLC1 | 0.578 | 0.000 | 0.000 | 0.000 |
| CYLC1 | 0.565 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Calyx、Perinuclear theca |
| PubMed | 10/20 | 8 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **72/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Calyx、Perinuclear theca
- 建议验证精子 IF 文献定位
