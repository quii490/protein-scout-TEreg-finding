---
type: sperm-protein-evaluation
gene: "FLACC1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# FLACC1 — 精子模块评估

## 1. 基本信息
- **基因:** FLACC1
- **Ensembl:** ENSG00000155749
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 6 篇
- **精子定位部位:** Annulus、Connecting piece、Mid piece (3 个)
- **UniProt Subcellular Location:** GO: cytoplasm; cytoplasmic vesicle; outer dense fiber; sperm flagellum

## 2. HPA 精子定位证据
- **来源:** Annulus、Connecting piece、Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000155749-FLACC1
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/35793/2199_C5_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/35793/2199_C5_41_blue_red_green.jpg)
![](https://images.proteinatlas.org/35793/390_B1_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/35793/390_B1_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/35793/1844_A7_32_blue_red_green.jpg)
![](https://images.proteinatlas.org/35793/377_B1_1_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: H7C1T5 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: FLACC1。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 6 篇 (极低研究量)
- *关键文献待人工调研。*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: H7C1T5)。参见 https://alphafold.ebi.ac.uk/entry/H7C1T5
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| FLACC1 | 0.864 | 0.000 | 0.000 | 0.000 |
| WDR38 | 0.634 | 0.000 | 0.000 | 0.000 |
| WDR38 | 0.633 | 0.000 | 0.000 | 0.000 |
| FLACC1 | 0.604 | 0.000 | 0.000 | 0.000 |
| ECT2L | 0.594 | 0.000 | 0.000 | 0.000 |
| C2CD6 | 0.517 | 0.000 | 0.000 | 0.000 |
| FLACC1 | 0.506 | 0.000 | 0.000 | 0.000 |
| ZNF404 | 0.480 | 0.000 | 0.000 | 0.000 |
| ZNF404 | 0.480 | 0.000 | 0.000 | 0.000 |
| ATP6AP1L | 0.476 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| SULT2B1 | two hybrid pooling approach | 16189514 |
| SULT2B1 | two hybrid array | 19060904 |
| SULT2B1 | two hybrid pooling approach | 19060904 |
| INO80B | two hybrid prey pooling approach | 32296183 |
| LONRF1 | two hybrid array | 32296183 |
| INO80B | two hybrid array | 32296183 |
| SULT2B1 | two hybrid array | 32296183 |
| TEPSIN | validated two hybrid | 32296183 |
| SSX4 | validated two hybrid | 32296183 |
| LONRF1 | validated two hybrid | 32296183 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 18/20 | 3 部位: Annulus、Connecting piece、Mid piece |
| PubMed | 10/20 | 6 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **75/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Annulus、Connecting piece、Mid piece
- 建议验证精子 IF 文献定位
