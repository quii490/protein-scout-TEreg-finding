---
type: sperm-protein-evaluation
gene: "TCTE1"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# TCTE1 — 精子模块评估

## 1. 基本信息
- **基因:** TCTE1
- **蛋白名称:** Dynein regulatory complex subunit 5
- **Ensembl:** ENSG00000146221
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 19 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** Cell projection, cilium, flagellum; Cytoplasm, cytoskeleton, flagellum axoneme

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000146221-TCTE1
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/31739/333_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/31739/333_A9_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/31739/2195_H9_26_blue_red_green.jpg)
![](https://images.proteinatlas.org/31739/331_A9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/31739/2195_H9_6_blue_red_green.jpg)
![](https://images.proteinatlas.org/31739/330_A9_2_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q5JU00 — Dynein regulatory complex subunit 5.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: DRC5, Leu-rich_rpt, LRR_dom_sf。
Pfam: LRR_6。

## 4. PubMed 文献证据
- **文献数:** 19 篇 (低研究量)
- 1. PMID 38650655: Effects of Tcte1 knockout on energy chain transportation and spermatogenesis: implications for male infertility. (2024) *Hum Reprod Open*
2. PMID 35388187: Bi-allelic variants in human TCTE1/DRC5 cause asthenospermia and male infertility. (2022 Jun) *Eur J Hum Genet*
3. PMID 28630322: TCTE1 is a conserved component of the dynein regulatory complex and is required for motility and metabolism in mouse spermatozoa. (2017 Jul 3) *Proc Natl Acad Sci U S A*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q5JU00)。参见 https://alphafold.ebi.ac.uk/entry/Q5JU00
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| CCDC65 | 0.933 | 0.000 | 0.000 | 0.000 |
| DYNLT1 | 0.904 | 0.000 | 0.000 | 0.000 |
| TCP10 | 0.894 | 0.000 | 0.000 | 0.000 |
| DRC7 | 0.849 | 0.000 | 0.000 | 0.000 |
| DRC7 | 0.843 | 0.000 | 0.000 | 0.000 |
| TCTE1 | 0.838 | 0.000 | 0.000 | 0.000 |
| TCTE1 | 0.837 | 0.000 | 0.000 | 0.000 |
| TCP10 | 0.831 | 0.000 | 0.000 | 0.000 |
| TCTE1 | 0.794 | 0.000 | 0.000 | 0.000 |
| TCTE1 | 0.785 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 8/20 | 19 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
