---
type: sperm-protein-evaluation
gene: "TTC29"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# TTC29 — 精子模块评估

## 1. 基本信息
- **基因:** TTC29
- **Ensembl:** ENSG00000137473
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 15 篇
- **精子定位部位:** Principal piece (1 个)
- **UniProt Subcellular Location:** GO: sperm flagellum; sperm principal piece

## 2. HPA 精子定位证据
- **来源:** Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000137473-TTC29
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/37006/2199_C3_26_blue_red_green.jpg)
![](https://images.proteinatlas.org/37006/2199_C3_11_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: A0ACI8RY79 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: 无注释。
Pfam: 无注释。

## 4. PubMed 文献证据
- **文献数:** 15 篇 (低研究量)
- 1. PMID 37934199: Novel axonemal protein ZMYND12 interacts with TTC29 and DNAH1, and is required for male fertility and flagellum function. (2023 Nov 7) *Elife*
2. PMID 31735292: Mutations in TTC29, Encoding an Evolutionarily Conserved Axonemal Protein, Result in Asthenozoospermia and Male Infertility. (2019 Dec 5) *Am J Hum Genet*
3. PMID 22878202: Regulation of rat tetratricopeptide repeat domain 29 gene expression by follicle-stimulating hormone. (2012) *Biosci Biotechnol Biochem*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0ACI8RY79)。参见 https://alphafold.ebi.ac.uk/entry/A0ACI8RY79
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| DNAH1 | 0.958 | 0.000 | 0.000 | 0.000 |
| TTC29 | 0.826 | 0.000 | 0.000 | 0.000 |
| DNAH1 | 0.790 | 0.000 | 0.000 | 0.000 |
| CFAP43 | 0.788 | 0.000 | 0.000 | 0.000 |
| TTC29 | 0.778 | 0.000 | 0.000 | 0.000 |
| CFAP43 | 0.774 | 0.000 | 0.000 | 0.000 |
| TTC29 | 0.771 | 0.000 | 0.000 | 0.000 |
| TTC29 | 0.763 | 0.000 | 0.000 | 0.000 |
| TTC29 | 0.742 | 0.000 | 0.000 | 0.000 |
| TTC21A | 0.741 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Principal piece |
| PubMed | 8/20 | 15 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 8/10 | 低 |

- **评分:** **65/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Principal piece
- 建议验证精子 IF 文献定位
