---
type: sperm-protein-evaluation
gene: "CROCC"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CROCC — 精子模块评估

## 1. 基本信息
- **基因:** CROCC
- **Ensembl:** ENSG00000058453
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 54 篇
- **精子定位部位:** Mid piece (1 个)
- **UniProt Subcellular Location:** GO: centrosome; ciliary basal body

## 2. HPA 精子定位证据
- **来源:** Mid piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000058453-CROCC
- **IF 图像:** 未获取

*HPA IF 图像未获取。已查询 HPA subcellular 页面。*


## 3. UniProt / GO-CC 精子定位证据
UniProt: A0A087WW81 — .
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Rootletin-like_CC。
Pfam: Rootletin。

## 4. PubMed 文献证据
- **文献数:** 54 篇 (中等研究量)
- 1. PMID 17971504: Tissue expression patterns identify mouse cilia genes. (2008 Jan 17) *Physiol Genomics*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: A0A087WW81)。参见 https://alphafold.ebi.ac.uk/entry/A0A087WW81
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| SASS6 | 0.999 | 0.000 | 0.000 | 0.000 |
| PCNT | 0.988 | 0.000 | 0.000 | 0.000 |
| CROCC | 0.981 | 0.000 | 0.000 | 0.000 |
| CNTLN | 0.969 | 0.000 | 0.000 | 0.000 |
| CEP250 | 0.933 | 0.000 | 0.000 | 0.000 |
| CNTLN | 0.923 | 0.000 | 0.000 | 0.000 |
| CNTLN | 0.922 | 0.000 | 0.000 | 0.000 |
| CEP250 | 0.908 | 0.000 | 0.000 | 0.000 |
| CNTLN | 0.887 | 0.000 | 0.000 | 0.000 |
| CROCC | 0.886 | 0.000 | 0.000 | 0.000 |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| BHLHE39 | tandem affinity purification | 21150319 |
| RNF152 | two hybrid array | 21988832 |
| HDAC8 | anti tag coimmunoprecipitation | 23752268 |
| TPM1 | anti tag coimmunoprecipitation | 28514442 |
| ESR1 | tandem affinity purification | 31527615 |
| ESR1 | tandem affinity purification | 31527615 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 14/20 | 单部位: Mid piece |
| PubMed | 7/20 | 54 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 6/10 | 中等 |

- **评分:** **62/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Mid piece
- 建议验证精子 IF 文献定位
