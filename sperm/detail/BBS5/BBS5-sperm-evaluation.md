---
type: sperm-protein-evaluation
gene: "BBS5"
module: sperm
status: sperm_low_priority
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# BBS5 — 精子模块评估

## 1. 基本信息
- **基因:** BBS5
- **蛋白名称:** BBSome complex member BBS5
- **Ensembl:** ENSG00000163093
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 80 篇
- **精子定位部位:** Acrosome、Perinuclear theca (2 个)
- **UniProt Subcellular Location:** Cell projection, cilium membrane; Cytoplasm; Cytoplasm, cytoskeleton, cilium basal body; Cytoplasm, cytoskeleton, microtubule organizing center, centrosome, centriolar satellite

## 2. HPA 精子定位证据
- **来源:** Acrosome、Perinuclear theca ✓
- **链接:** https://www.proteinatlas.org/ENSG00000163093-BBS5
- **IF 图像:** 已获取 (6 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (6 张 blue_red_green)。
![](https://images.proteinatlas.org/46125/2128_B9_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/46125/2200_F4_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/46125/736_E4_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/46125/736_E4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/46125/2168_D5_56_blue_red_green.jpg)
![](https://images.proteinatlas.org/46125/2200_F4_31_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: Q8N3I7 — BBSome complex member BBS5.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: BBL5, BBS5/fem-3, BBS5_PH, PH-like_dom_sf。
Pfam: BBL5。

## 4. PubMed 文献证据
- **文献数:** 80 篇 (较多研究量)
- 1. PMID 20301537: Bardet-Biedl Syndrome Overview. (1993) **
2. PMID 26867008: A Splice Variant of Bardet-Biedl Syndrome 5 (BBS5) Protein that Is Selectively Expressed in Retina. (2016) *PLoS One*
3. PMID 15137946: Comparative genomics identifies a flagellar and basal body proteome that includes the BBS5 human disease gene. (2004 May 14) *Cell*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: Q8N3I7)。参见 https://alphafold.ebi.ac.uk/entry/Q8N3I7
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| *STRING 无数据* | — | — | — | — |

### IntAct 实验验证 PPI (Detection 方法)
| Partner | Detection | PMID |
|---|---|---|
| BBS4 | tandem affinity purification | 17574030 |
| BBS1 | cosedimentation through density gradient | 17574030 |
| PTHB1 | anti tag coimmunoprecipitation | 17574030 |


## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 16/20 | 双部位: Acrosome、Perinuclear theca |
| PubMed | 6/20 | 80 篇 |
| PPI | 5/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 4/10 | 较多 |

- **评分:** **47/100**

## 8. 结论
**SPERM LOW PRIORITY**

## 9. 人工复核备注
- 精子部位: Acrosome、Perinuclear theca
- 建议验证精子 IF 文献定位
