---
type: sperm-protein-evaluation
gene: "CBY2"
module: sperm
status: sperm_candidate
date: 2026-06-22
tags: [protein-scout, sperm, evaluation]
---

# CBY2 — 精子模块评估

## 1. 基本信息
- **基因:** CBY2
- **蛋白名称:** Protein chibby homolog 2
- **Ensembl:** ENSG00000174015
- **抗体:** 未获取
- **IF 可靠性:** 未获取
- **PubMed:** 6 篇
- **精子定位部位:** Annulus、Calyx、Connecting piece、End piece、Mid piece、Principal piece (6 个)
- **UniProt Subcellular Location:** GO: cytoplasmic vesicle

## 2. HPA 精子定位证据
- **来源:** Annulus、Calyx、Connecting piece、End piece、Mid piece、Principal piece ✓
- **链接:** https://www.proteinatlas.org/ENSG00000174015-CBY2
- **IF 图像:** 已获取 (2 张)


<!-- SPERM_HPA_IF_START -->
**HPA IF 图像（2026-06-22）**: HPA subcellular 页面有 IF 图像 (2 张 blue_red_green)。
![](https://images.proteinatlas.org/39359/2229_B7_36_blue_red_green.jpg)
![](https://images.proteinatlas.org/39359/2229_B7_21_blue_red_green.jpg)
<!-- SPERM_HPA_IF_END -->



## 3. UniProt / GO-CC 精子定位证据
UniProt: E9PLB4 — Protein chibby homolog 2.
GO-CC 精子相关: 待进一步查询 UniProt subcellular location。
InterPro: Chibby_fam。
Pfam: Chibby。

## 4. PubMed 文献证据
- **文献数:** 6 篇 (极低研究量)
- 1. PMID 34290169: CRISPR/Cas9-mediated genome editing reveals 12 testis-enriched genes dispensable for male fertility in mice. (2022 May-Jun) *Asian J Androl*
2. PMID 38733656: Identification of a novel mutation in chibby family member 2 in a non-obstructive azoospermic patient. (2024 Jun) *Reprod Biol*
3. PMID 39961484: Comparative proteomics and phosphoproteomics analysis reveals differential sperm motility in Mediterranean buffalo semen. (2025 May 15) *J Proteomics*

## 5. AlphaFold / PAE / PDB / 结构域
AlphaFold 数据可用 (UniProt: E9PLB4)。参见 https://alphafold.ebi.ac.uk/entry/E9PLB4
PAE 图像暂无数据（未生成本地图片），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络
### STRING (人类, top 10)
| Partner | Combined | Exp | DB | Text |
|---|---|---|---|---|
| FAM187B | 0.751 | 0.000 | 0.000 | 0.000 |
| CBY2 | 0.712 | 0.000 | 0.000 | 0.000 |
| CBY3 | 0.708 | 0.000 | 0.000 | 0.000 |
| FAM187B | 0.650 | 0.000 | 0.000 | 0.000 |
| SPATA3 | 0.610 | 0.000 | 0.000 | 0.000 |
| KCTD4 | 0.605 | 0.000 | 0.000 | 0.000 |
| SPATA3 | 0.602 | 0.000 | 0.000 | 0.000 |
| CBY2 | 0.601 | 0.000 | 0.000 | 0.000 |
| SPEM1 | 0.588 | 0.000 | 0.000 | 0.000 |
| GTF2F2 | 0.572 | 0.000 | 0.000 | 0.000 |
*待 IntAct/BioGRID/humanPPI 补充。*

## 7. 评分表
| 维度 | 评分 | 依据 |
|---|---:|---|
| 精子定位 | 19/20 | 6 部位: Annulus、Calyx、Connecting piece、End piece、Mid piece、Principal piece |
| PubMed | 10/20 | 6 篇 |
| PPI | 18/20 | STRING |
| 结构 | 5/10 | AF available |
| 新颖性 | 10/10 | 极低 |

- **评分:** **77/100**

## 8. 结论
**SPERM CANDIDATE**

## 9. 人工复核备注
- 精子部位: Annulus、Calyx、Connecting piece、End piece、Mid piece、Principal piece
- 建议验证精子 IF 文献定位
