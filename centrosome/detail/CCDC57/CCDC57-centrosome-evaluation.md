---
type: centrosome-protein-evaluation
gene: "CCDC57"
module: centrosome
status: centrosome_candidate
date: 2026-06-08
tags: [protein-scout, centrosome, evaluation]
---

# CCDC57 — 中心体模块评估

## 1. 基本信息

- **基因:** CCDC57
- **Ensembl:** ENSG00000176155
- **HPA 来源:** 中心粒卫星
- **HPA 抗体:** HPA023315, HPA023326, HPA023342, HPA023344
- **IF 可靠性:** Approved
- **PubMed 文献总数:** 21 篇

## 2. HPA 中心体 / 中心粒卫星证据

- **HPA 来源:** 中心粒卫星 ✓
- **HPA 链接:** https://www.proteinatlas.org/ENSG00000176155-CCDC57
- **HPA 定位:** Microtubules, Centriolar satellite, Cytosol
- **IF 图像状态:** 已获取 (1 张, selected)


<!-- CENTROSOME_HPA_IF_START -->
**HPA IF 图像（2026-06-08）**: HPA subcellular 页面存在可用 IF 图像。
HPA 定位: Microtubules, Centriolar satellite, Cytosol。HPA IF 可靠性: approved。
来源: https://www.proteinatlas.org/ENSG00000176155-CCDC57/subcellular

![](https://images.proteinatlas.org/23315/182_A6_2_selected.jpg)
<!-- CENTROSOME_HPA_IF_END -->


## 3. UniProt / GO-CC 中心体证据

*待 UniProt/GO-CC 完整采集。需人工审核。*

- 初步: HPA 标注支持 中心粒卫星 定位。

## 4. PubMed 文献证据

- **文献总数:** 21 篇
- **研究量评估:** 低研究量
- 1. PMID 32402286: CCDC57 Cooperates with Microtubules and Microcephaly Protein CEP63 and Regulates Centriole Duplication and Mitotic Progression. (2020 May 12) *Cell Rep*
2. PMID 39592607: Directional ciliary beats across epithelia require Ccdc57-mediated coupling between axonemal orientation and basal body polarity. (2024 Nov 26) *Nat Commun*
3. PMID 41758249: Ccdc57 regulates cilia and left-right patterning in Xenopus. (2026 Feb 15) *Biol Open*

## 5. AlphaFold / PAE / PDB / 结构域

pLDDT 数据可用 (UniProt: Q2TAC2)。PDB 模型: https://alphafold.ebi.ac.uk/files/AF-Q2TAC2-F1-model_v6.pdb

*InterPro: CCDC57*
*Pfam: *待 Pfam 查询**

PAE 图像暂无数据（未生成本地图片或未可靠获取），结构判断基于 AlphaFold pLDDT 统计。

## 6. PPI / 蛋白互作网络

### STRING (人类, top 10)

| Partner | Combined Score | Experimental | Database | Textmining |
|---|---|---|---|
| CCDC57 | 0.642 | 0.000 | 0.000 | 0.000 |
| CCDC57 | 0.543 | 0.000 | 0.000 | 0.000 |
| CCDC57 | 0.528 | 0.000 | 0.000 | 0.000 |
| CCDC57 | 0.521 | 0.000 | 0.000 | 0.000 |
| AMZ1 | 0.519 | 0.000 | 0.000 | 0.000 |
| NKAIN4 | 0.519 | 0.000 | 0.000 | 0.000 |
| SFI1 | 0.509 | 0.000 | 0.000 | 0.000 |
| RDM1 | 0.492 | 0.000 | 0.000 | 0.000 |
| CCDC57 | 0.491 | 0.000 | 0.000 | 0.000 |
| CCDC57 | 0.479 | 0.000 | 0.000 | 0.000 |

*待 IntAct / BioGRID / humanPPI 补充。*

## 7. 中心体模块评分表

| 维度 | 评分 | 依据 |
|---|---:|---|
| 中心体证据 | 14/20 | HPA 中心粒卫星标注。定位: Microtubules, Centriolar satellite, Cytosol |
| PubMed/文献 | 8/20 | 21 篇文献 |
| PPI/互作网络 | 15/20 | STRING 互作数据 |
| 结构/结构域 | 5/10 | 待结构数据采集 |
| 新颖性/特异性 | 8/10 | 低研究量 |

- **最终评分:** **61/100**

## 8. 最终结论

**CENTROSOME CANDIDATE**

⚠️ *此为自动生成初步评估。UniProt/GO-CC、PDB/结构域、IntAct/BioGRID、关键文献等维度需人工补充完善。*

## 9. 人工复核备注

- HPA 来源: 中心粒卫星
- 抗体: HPA023315, HPA023326, HPA023342, HPA023344（IF 可靠性: Approved）
- 建议: 核实中心体 IF 文献定位
- 如 IF 图像质量不佳，检查 HPA 是否有替代抗体
