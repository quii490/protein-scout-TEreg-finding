---
type: centrosome-protein-evaluation
gene: "PCNT"
module: centrosome
status: centrosome_eliminated
date: 2026-06-08
tags: [protein-scout, centrosome, evaluation, eliminated]
---

# PCNT — Centrosome Module Evaluation (ELIMINATED)

## 1. 基本信息

- **Gene:** PCNT
- **Ensembl:** ENSG00000160299
- **HPA seed source:** both
- **PubMed total:** 259 papers ⚠️ **EXCEEDS THRESHOLD (>100)**

## 2. ELIMINATED

**Reason:** PubMed total = 259 (>100 threshold). Eliminated per centrosome module PubMed > 100 rule.

**Centrosome-specific papers:** N/A

## 3. HPA Seed Evidence

- HPA source: both
- HPA URL: https://www.proteinatlas.org/ENSG00000160299-PCNT

## 4. Notes

- No detailed evaluation performed (auto-eliminated by PubMed count).
- To re-evaluate, manually review whether PubMed count is inflated by aliases or non-centrosome literature.
