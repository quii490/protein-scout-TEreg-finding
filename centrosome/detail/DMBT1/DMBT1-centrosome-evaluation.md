---
type: centrosome-protein-evaluation
gene: "DMBT1"
module: centrosome
status: centrosome_eliminated
date: 2026-06-08
tags: [protein-scout, centrosome, evaluation, eliminated]
---

# DMBT1 — Centrosome Module Evaluation (ELIMINATED)

## 1. 基本信息

- **Gene:** DMBT1
- **Ensembl:** ENSG00000187908
- **HPA seed source:** satellite
- **PubMed total:** 480 papers ⚠️ **EXCEEDS THRESHOLD (>100)**

## 2. ELIMINATED

**Reason:** PubMed total = 480 (>100 threshold). Eliminated per centrosome module PubMed > 100 rule.

**Centrosome-specific papers:** N/A

## 3. HPA Seed Evidence

- HPA source: satellite
- HPA URL: https://www.proteinatlas.org/ENSG00000187908-DMBT1

## 4. Notes

- No detailed evaluation performed (auto-eliminated by PubMed count).
- To re-evaluate, manually review whether PubMed count is inflated by aliases or non-centrosome literature.
