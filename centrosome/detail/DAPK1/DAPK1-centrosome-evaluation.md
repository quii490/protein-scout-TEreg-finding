---
type: centrosome-protein-evaluation
gene: "DAPK1"
module: centrosome
status: centrosome_eliminated
date: 2026-06-08
tags: [protein-scout, centrosome, evaluation, eliminated]
---

# DAPK1 — Centrosome Module Evaluation (ELIMINATED)

## 1. 基本信息

- **Gene:** DAPK1
- **Ensembl:** ENSG00000196730
- **HPA seed source:** centrosome
- **PubMed total:** 800 papers ⚠️ **EXCEEDS THRESHOLD (>100)**

## 2. ELIMINATED

**Reason:** PubMed total = 800 (>100 threshold). Eliminated per centrosome module PubMed > 100 rule.

**Centrosome-specific papers:** N/A

## 3. HPA Seed Evidence

- HPA source: centrosome
- HPA URL: https://www.proteinatlas.org/ENSG00000196730-DAPK1

## 4. Notes

- No detailed evaluation performed (auto-eliminated by PubMed count).
- To re-evaluate, manually review whether PubMed count is inflated by aliases or non-centrosome literature.
