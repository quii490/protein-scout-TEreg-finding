...
**评价**: HPA 主定位为 Nuclear bodies。标注定位包括: Nucleoplasm, Nuclear bodies, Cytosol。抗体可靠性评级: Supported。HPA IF 原图未可靠获取（HPA检索页无可用的subcellular IF原图，仅有display image列表）。核定位基于HPA localization/reliability + UniProt + GO-CC。，显示核内信号分布。
