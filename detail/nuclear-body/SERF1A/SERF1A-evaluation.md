...
SERF1A（Small EDRK-Rich Factor 1A, 110 aa, ~12.3 kDa）是一个微小的、富含正电荷(EDRK: Glu-Asp-Arg-Lys)残基的核蛋白。UniProt实验注释显示该蛋白通过GO IMP位于核内（PMID:31034892），HPA在AF22和HEK293细胞中检测到核体（Nuclear bodies）染色。其极小的分子量（12.3 kDa）使其可自由被动扩散通过核孔复合物（排除限~40 kDa）。SERF1A被预测为内在无序蛋白（IDP），可能通过液-液相分离（LLPS）形成聚集体分布于核体。

SERF1A的功能被报道与肌萎缩侧索硬化（ALS）和脊髓性肌萎缩（SMA）的修饰因子有关。在SMA中，SERF1A作为SMN蛋白的修饰因子影响疾病的严重程度。值得注意的是，SMA致病基因SMN1的缺失导致小核核糖核蛋白（snRNP）装配缺陷和剪接体异常——这可能导致内含子保留事件增加，包括TE衍生序列的不正常剪接。此外，SERF1A的正电荷富集（EDRK重复）可能赋予其非特异性RNA结合能力，若在核体中结合TE衍生RNA，可能形成RNA-蛋白聚集体限制其可翻译性或逆转座。

从TE调控角度，SERF1A的核体定位及其与剪接体疾病SMA的关联使其成为研究核体TE RNA隔离功能的潜在候选。实验上可通过SERF1A RIP-seq鉴定其RNA靶标（特别是TE衍生转录本），结合SMA患者样本和iPSC衍生运动神经元模型评估TE表达变化。
