---
type: protein-evaluation
gene: "RACGAP1"
date: 2026-05-30
tags: [protein-scout, rejected]
status: rejected
---

## RACGAP1 核蛋白评估报告（淘汰）

### 1. 基本信息

| 项目 | 内容 |
|------|------|
| 基因名 | RACGAP1 |
| 蛋白名称 | Rac GTPase activating protein 1 | Rac GTPase-activating protein 1 |
| UniProt ID | Q9H0H5 |
| 蛋白大小 | 632 aa |
| 核定位分数 | 4 |
| PubMed 总数 | 7590 |
| 评估日期 | 2026-05-30 |

### 2. 淘汰原因

**淘汰类型**: PubMed 超过阈值

**详细理由**: PubMed 发表数 7590 篇，超过 100 篇阈值，研究领域过于拥挤

#
> **Protein Atlas IF**: 暂无数据（Pending cell analysis），核定位基于 UniProt + GO。

## 3. 关键数据

| 指标 | 数值 |
|------|------|
| PubMed 总数 (strict sum) | 7590 |
| PubMed 最大值 | 7431 |
| PubMed 近 5 年 | 1107 |
| 核定位分数 (weighted max) | 4 |
| 核定位分级 | Tier1_conserved_high_confidence |
| Research hotness | 10.3745 |

### 4. 结论

该基因不满足蛋白评估的基本筛选条件（PubMed ≤ 100 且核定位 > 3），予以淘汰。
