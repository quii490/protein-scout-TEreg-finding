...
| Data Source | Curated public database evidence and existing report-level annotations |
...
| Cross-Validation Bonus | -- | -- | **+0** | Consistent ER annotation; nuclear support absent from curated localization sources |
| **Raw Total** | | | **72.0/180** | |
...
| Metric | Value |
|--------|-------|
| PubMed strict query | `"SELENOF"[Title/Abstract] OR "SEP15"[Title/Abstract]` |
| PubMed strict estimated count | ~40-60 articles |
| PubMed broad query | `"SELENOF" OR "SEP15" OR "Selenoprotein F"` |
| PubMed broad estimated count | Moderate; dominated by selenium biology and ER quality-control studies |
...
| pLDDT / PAE | AlphaFold model expected to be most confident across the compact thioredoxin-like domain; PAE image/metadata should be interpreted as supportive structural context rather than evidence for nuclear function |
| PDB Entries | 0 (no experimental structures) |
| Domain | Thioredoxin-like fold |

AlphaFold prediction available. The thioredoxin-like domain is a well-characterized protein fold typically predicted with high confidence. PAE/pLDDT evidence supports the compact ER redox-fold interpretation, but it does not create nuclear localization or TE-regulatory relevance. No experimental structures. Score 5/10.
...
### 9. TE-Regulator Relevance Reasoning

SELENOF has very low relevance to TE regulation. Its localization, domain architecture, and interaction context all point to ER protein quality control rather than nuclear transcription, chromatin regulation, RNA processing, DNA repair, or TE-silencing biology. The thioredoxin-like selenoprotein fold is compatible with ER redox chemistry and disulfide quality control, not with DNA/RNA binding or chromatin reader activity. UGGT1/UGCGL1-linked retention further supports an ER lumen role. This evidence explains why the current rejected status is based on biological localization and function rather than a data retrieval artifact.
...
- Note: Evidence summary compiled from curated public database annotations and existing report-level sources
