---
type: protein-evaluation
gene: "SOBP"
date: 2026-05-30
tags: [protein-scout, rejected]
status: rejected
---

## SOBP 核蛋白评估报告（淘汰）

### 1. 基本信息

| 项目 | 内容 |
|------|------|
| 基因名 | SOBP |
| 蛋白名称 | sine oculis binding protein homolog | sine oculis binding protein |
| UniProt ID | A7XYQ1 |
| 蛋白大小 | 873 aa |
| 核定位分数 | 4 |
| PubMed 总数 | 311 |
| 评估日期 | 2026-05-30 |

### 2. 淘汰原因

**淘汰类型**: PubMed 超过阈值

**详细理由**: PubMed 发表数 311 篇，超过 100 篇阈值，研究领域过于拥挤

#
> **Protein Atlas IF**: 暂无数据（Pending cell analysis），核定位基于 UniProt + GO。

## 3. 关键数据

| 指标 | 数值 |
|------|------|
| PubMed 总数 (strict sum) | 311 |
| PubMed 最大值 | 258 |
| PubMed 近 5 年 | 65 |
| 核定位分数 (weighted max) | 4 |
| 核定位分级 | Tier1_conserved_high_confidence |
| Research hotness | 5.9155 |

### 4. 结论

该基因不满足蛋白评估的基本筛选条件（PubMed ≤ 100 且核定位 > 3），予以淘汰。
