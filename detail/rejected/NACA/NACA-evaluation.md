---
type: protein-evaluation
gene: "NACA"
date: 2026-05-30
tags: [protein-scout, rejected]
status: rejected
---

## NACA 核蛋白评估报告（淘汰）

### 1. 基本信息

| 项目 | 内容 |
|------|------|
| 基因名 | NACA |
| 蛋白名称 | nascent polypeptide associated complex subunit alpha | nascent polypeptide-associated complex alpha polypeptide |
| UniProt ID | E9PAV3 |
| 蛋白大小 | 2078 aa |
| 核定位分数 | 4 |
| PubMed 总数 | 656 |
| 评估日期 | 2026-05-30 |

### 2. 淘汰原因

**淘汰类型**: PubMed 超过阈值

**详细理由**: PubMed 发表数 656 篇，超过 100 篇阈值，研究领域过于拥挤

#
HPA IF 图像已重新获取并嵌入（见下方 HPA IF 图像修正块）；此前“暂无/未可靠获取 IF”的表述为采集失败导致的误报。

## 3. 关键数据

| 指标 | 数值 |
|------|------|
| PubMed 总数 (strict sum) | 656 |
| PubMed 最大值 | 498 |
| PubMed 近 5 年 | 89 |
| 核定位分数 (weighted max) | 4 |
| 核定位分级 | Tier1_conserved_high_confidence |
| Research hotness | 7.7422 |

### 4. 结论

该基因不满足蛋白评估的基本筛选条件（PubMed ≤ 100 且核定位 > 3），予以淘汰。

<!-- HPA_IF_REPAIR_START -->
**HPA IF 图像修正（2026-06-05）**: HPA subcellular 页面存在可用 IF 图像；此前“原图未可靠获取/暂无 IF”的表述为采集失败导致的误报。HPA 定位: Cytosol (approved)。来源: https://www.proteinatlas.org/ENSG00000196531-NACA/subcellular

![](https://images.proteinatlas.org/73152/1437_A6_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/73152/1437_A6_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/73152/1564_G4_2_blue_red_green.jpg)
![](https://images.proteinatlas.org/73152/1564_G4_3_blue_red_green.jpg)
![](https://images.proteinatlas.org/73152/1668_A6_1_blue_red_green.jpg)
![](https://images.proteinatlas.org/73152/1668_A6_2_blue_red_green.jpg)
<!-- HPA_IF_REPAIR_END -->
