...
| HPA (IF) | HPA IF 原图未可靠获取（HPA检索页无可用的subcellular IF原图）。核定位基于HPA localization/reliability + UniProt + GO-CC。 — HPA 无 IF 图像 | — |

**HPA IF 状态**: HPA IF 原图未可靠获取（HPA检索页无可用的subcellular IF原图）。核定位基于HPA localization/reliability + UniProt + GO-CC。 — HPA 暂无 IF 原图数据。HPA 数据库 GO-CC 注释 nucleoplasm IDA:HPA 说明 HPA 抗体染色支持核定位，但未获取图像。核定位基于 GO-CC IDA:HPA。
