...
EXOC3L2（Exocyst complex component 3-like protein 2）属于exocyst复合体相关蛋白家族。InterPro结构域注释显示该蛋白含IPR010326（Exocyst complex component 3-like）和IPR042532结构域，均对应Pfam PF06046。AlphaFold v6预测整体pLDDT=73.5，其中高置信度残基（pLDDT>90）占38.5%，有序区域（pLDDT>70）占65.8%。PAE图表明蛋白内部存在多个相对独立折叠的结构域模块，但低置信度区域（27.8%残基pLDDT<50）暗示部分片段可能为固有无序区域（IDR），这些IDR区域在核质定位和蛋白-蛋白相互作用中可能发挥关键作用。

PPI网络分析揭示EXOC3L2与exocyst核心复合体的系统关联。STRING数据库显示其与EXOC4（combined score=0.951, experimental=0.720）、EXOC7（0.868）、EXOC2（0.864）、EXOC5（0.847）、EXOC8（0.847）、EXOC1（0.832）、EXOC6（0.818）等exocyst亚基高度关联，提示EXOC3L2通过EXOC3L家族参与exocyst介导的囊泡栓系过程。实验验证方面，IntAct记录显示EXOC3L2与ATG16L1存在抗标签免疫共沉淀互作（PMID:31015422），暗示其可能参与自噬体形成调控；与SIAH1和HSPA8的共免疫沉淀互作（PMID:28514442）提示其可能参与泛素化-蛋白酶体降解途径的调控。值得注意的是，HPA IF定位于高尔基体和囊泡，额外定位于核质和核仁，但exocyst经典功能以胞质膜运输为主，核内定位的潜在功能尚不明确。

从结构域架构角度看，EXOC3L2不含经典的DNA结合域或染色质修饰域，其核定位可能依赖于蛋白-蛋白相互作用介导的核转位机制或IDR区域的核定位信号。BioGRID记录的互作伙伴MDC1（DNA损伤检查点蛋白）为此提供线索，EXOC3L2可能作为DNA损伤应答（DDR）的辅助因子被招募入核。

该蛋白的研究现状极为稀疏，PubMed strict仅11篇文献，其中3篇与纤毛病相关（PMID:27894351），3篇与阿尔茨海默病相关（PMID:23663385、PMID:26680604），1篇报道双等位基因突变导致脑-肾-血液综合征（PMID:30327448）。这些临床关联提示EXOC3L2在神经发育和纤毛生物学中的重要功能，但其分子机制、核内功能和在TE调控中的潜在角色几乎完全未知，构成了一个具有高探索价值的候选方向。
...
Source: https://www.proteinatlas.org/ENSG00000283632-EXOC3L2/interaction
