...
HPA IF 原图在本次采集中未可靠获取。image_status 为 "no_image_detected"，未返回 subcellular IF display images。Image URLs 中包含 `_red_green` 模式的 IF 原始图像链接，提示 HPA 数据库中存有 IF 图像。核定位依据 HPA subcellular_location record (Nucleoplasm + Cytosol, Supported reliability) 及 GO-CC 多重验证。
...
HPA IF 状态: image_status="no_image_detected"，IF display images 未获取。HPA 定位记录 (Nucleoplasm+Cytosol, Supported) 用于核定位判断。
