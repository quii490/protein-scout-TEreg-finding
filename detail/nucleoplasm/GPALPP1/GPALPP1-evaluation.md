---
type: protein-evaluation
gene: "GPALPP1"
date: 2026-06-03
tags: [protein-scout, nuclear-protein, evaluation]
status: scored
---

## GPALPP1 核蛋白评估报告 (Full Re-evaluation)

### 1. 基本信息

| 项目 | 内容 |
|------|------|
| 基因名 / 别名 | GPALPP1 / KIAA1704, LSR7 |
| 蛋白名称 | GPALPP motifs-containing protein 1 |
| 蛋白大小 | 340 aa / 38.1 kDa |
| UniProt ID | Q8IXQ4 |
| 评估日期 | 2026-06-03 |

### 2. 评分总览

| 维度 | 得分 | 满分 | 加权后 | 关键证据摘要 |
|------|------|------|--------|-------------|
| 核定位特异性 | 7/10 | ×4 | 28 | HPA: Nucleoli rim; 额外: Nucleoplasm; UniProt: 无注释 |
| 蛋白大小 | 10/10 | ×1 | 10 | 340 aa / 38.1 kDa |
| 研究新颖性 | 10/10 | ×5 | 50 | PubMed strict=0 篇 (≤20→10) |
| 三维结构 | 6/10 | ×3 | 18 | AlphaFold v6 pLDDT=69.4; PDB: 无 |
| 调控结构域 | 7/10 | ×2 | 14 | InterPro: IPR022226, IPR046331; Pfam: PF12572 |
| PPI 网络 | 3/10 | ×3 | 9 | STRING 15 partners; IntAct 15 interactions |
| 互证加分 | — | max +3 | 0.5 | PDB+AF+STRING+IntAct cross-validation |
| **原始总分** | | | **129.5/180** | |
| **归一化总分** | | | **71.9/100** | |

### 3. 详细分析

#### 3.1 核定位证据

| 来源 | 定位 | 可信度 |
|------|------|--------|
| Protein Atlas (IF) | Nucleoli rim; 额外: Nucleoplasm | Approved |
| UniProt | 无注释 | Swiss-Prot/TrEMBL |

HPA IF 图像已重新获取并嵌入（见下方 HPA IF 图像修正块）；此前“暂无/未可靠获取 IF”的表述为采集失败导致的误报。

**GO Cellular Component**:
- 无 GO-CC 注释

**结论**: 主要核定位，HPA 可靠性良好，有辅助数据源支持。

#### 3.2 蛋白大小评估

**评价**: 大小适中（200-800 aa），适合常规生化实验和结构解析。

#### 3.3 研究现状

| 指标 | 数值 |
|------|------|
| PubMed strict count | 0 |
| PubMed broad count | 0 |
| 别名(未计入scoring) | 无 |

**关键文献**:
无关键文献数据。

**评价**: 极度新颖，几乎未被系统研究（PubMed ≤20篇）。

#### 3.4 三维结构分析

| 指标 | 数值 |
|------|------|
| AlphaFold 版本 | v6 |
| AlphaFold 平均 pLDDT | 69.4 |
| 高置信度残基 (pLDDT>90) 占比 | 11.2% |
| 置信残基 (pLDDT 70-90) 占比 | 40.9% |
| 中等置信 (pLDDT 50-70) 占比 | 29.1% |
| 低置信 (pLDDT<50) 占比 | 18.8% |
| 有序区域 (pLDDT>70) 占比 | 52.1% |
| 可用 PDB 条目 | 无 |

**PAE 图像说明**: AlphaFold PAE 图像已重新获取并嵌入（见下方 PAE 图像修正块）；结构判断仍结合 pLDDT 与 PAE 综合判断。

**评价**: AlphaFold 预测质量有限（pLDDT=69.4），有序残基占 52.1%。

#### 3.5 结构域分析

| 来源 | 结构域 |
|------|--------|
| InterPro/Pfam | InterPro: IPR022226, IPR046331; Pfam: PF12572 |

**染色质调控潜力分析**: 存在已知结构域注释，可作为功能研究的结构基础。

#### 3.6 PPI 网络

**STRING 预测互作** (combined score >0.4):

| Partner | Combined Score | Experimental | 功能类别 |
|---------|---------------|--------------|---------|
| ARL6IP4 | 0.621 | 0.620 | — |
| CCDC82 | 0.597 | 0.000 | — |
| LRRC2 | 0.546 | 0.000 | — |
| WDR6 | 0.541 | 0.000 | — |
| TCEANC2 | 0.533 | 0.000 | — |
| SNIP1 | 0.516 | 0.385 | — |
| TBL2 | 0.504 | 0.000 | — |
| TMCO1 | 0.493 | 0.000 | — |
| XRRA1 | 0.485 | 0.000 | — |
| GTPBP8 | 0.475 | 0.000 | — |

**实验验证互作** (IntAct):

| Partner | 方法 | PMID |
|---------|------|------|
| PPIL1 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| CDK8 | psi-mi:"MI:1314"(proximity-dependent biotin identi | pubmed:29568061|imex:IM-26301 |
| ARL6IP4 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| ZNF816 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| PIP4K2A | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| CDKL5 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| ZNF786 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| FUOM | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| ZNF311 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |
| CLK2 | psi-mi:"MI:0007"(anti tag coimmunoprecipitation) | pubmed:28514442|doi:10.1038/na |

**PPI 互证分析**:
- STRING + IntAct 均有数据
- STRING partners: 15，IntAct interactions: 15
- 调控相关比例: 0 / 15 = 0%

**评价**: STRING 15 个预测互作，IntAct 15 个实验互作。调控相关配体占比 0%。

#### 3.7 多库互证

| 维度 | 来源 | 结果 | 是否一致 |
|------|------|------|----------|
| 三维结构 | AlphaFold pLDDT=69.4 + PDB: 无 | pLDDT=69.4, v6 | 仅预测 |
| 定位 | UniProt + HPA | 无注释 / Nucleoli rim; 额外: Nucleoplasm | 待确认 |
| PPI | STRING + IntAct | 15 + 15 interactions | 数据充分 |

**互证加分明细**:
- PDB + AlphaFold 双源验证: +0
- 多库定位一致: +0
- STRING + IntAct 双源验证: +0.5
- 结构域 + AlphaFold 质量: +0
- PDB 多条目覆盖: +0
**总分**: +0.5 / max +3

### 4. 总体评价

**推荐等级**: ⭐⭐⭐⭐

**核心优势**:
1. GPALPP1 — GPALPP motifs-containing protein 1，极度新颖，几乎未被系统研究（PubMed ≤20篇）。
2. 蛋白大小340 aa，大小适中（200-800 aa），适合常规生化实验和结构解析。

**风险/不确定性**:
1. PubMed 0 篇，研究基础极有限，功能注释不完整
2. AlphaFold 预测质量一般（pLDDT=69.4），需要更多实验结构验证

**下一步建议**:
- [ ] 查阅最新关键文献补充研究背景
- [ ] 获取 Protein Atlas IF 图像确认亚细胞定位
- [ ] 设计体外实验验证核定位及潜在调控功能


### 深度机制分析

GPALPP1（GPALPP motifs-containing protein 1）是一个功能完全未知的核仁相关蛋白，其结构域注释揭示了进化保守但功能未表征的模块：InterPro注释为IPR022226（GPALPP motif）和IPR046331（GPALPP-like domain），Pfam对应PF12572（GPALPP）。"GPALPP" motif的名称暗示其包含连续的氨基酸序列特征（Gly-Pro-Ala-Leu-Pro-Pro），可能形成特定的二级结构（如脯氨酸富集螺旋或polyproline型螺旋）。AlphaFold v6预测整体pLDDT=69.4，有序区域仅占52.1%，近半数蛋白可能处于天然无序状态。这种高比例的无序区域特征与许多核仁蛋白（如nucleolin、nucleophosmin）相似——核仁作为无膜细胞器，其组分的液-液相分离（LLPS）倾向依赖于IDR区域的低复杂度序列。

PPI网络提供了GPALPP1功能推断的核心线索。IntAct实验验证的互作中，CDK8（周期蛋白依赖性激酶8，PMID:29568061, BioID proximity labeling）的记录尤为关键：CDK8是Mediator复合体的核心激酶亚基，在转录起始和延伸调控中发挥全局性作用。GPALPP1-CDK8的邻近标记互作共同出现于同一蛋白复合体微环境，提示GPALPP1可能定位于Mediator复合体的核质/核仁边界区域。CLK2（CDC样激酶2，PMID:28514442, co-IP）的互作进一步支持这一方向——CLK2是SR蛋白磷酸化激酶，调控mRNA前体剪接。此外，PIP4K2A（磷脂酰肌醇5-磷酸4-激酶，PMID:28514442）和ARL6IP4（ADP-核糖基化因子样蛋白6互作蛋白4，STRING experimental=0.620）的互作分别指向核内磷酸肌醇信号和核质转运调控。

BioGRID录入了额外的核心调控因子互作：CTBP2（C末端结合蛋白2，转录共抑制子）、XPO1（Exportin-1/CRM1，核输出受体）、EWSR1（Ewing肉瘤断点区蛋白1，FET蛋白家族RNA结合蛋白）、THOC2（THO复合体亚基，mRNA出核）和SNIP1（Smad核互作蛋白1）。这些互作汇聚成一个清晰的核调控功能簇：GPALPP1似乎坐落于Mediator-转录-剪接-核质运输的交叉节点。

GPALPP1最引人注目的特征是PubMed严格计数为0（broad=0），是50个候选蛋白中唯一无任何文献记录的成员。仅有2篇broad文献（PMID:38522031：人乳恒牙胚发育时间蛋白特征；PMID:37330465：特应性皮炎焦亡相关诊断模型）通过间接关联被记录。这种极端的研究空白（即使以Protein Scout的标准衡量）与HPA IF显示的清晰核仁边缘定位（Nucleoli rim, Approved）及丰富的调控相关PPI形成鲜明对比。GPALPP1-CDKL5的互作也值得关注——CDKL5磷酸化MeCP2（甲基CpG结合蛋白2），后者是DNA甲基化解读器和TE沉默的关键因子。因此，GPALPP1作为核仁-核质界面的新型调控因子，在TE调控研究中具有极高的新颖性得分和合理的机制基础，强烈推荐作为优先验证靶点。

### PPI 互作网络

| 互作伙伴 | 来源 | 评分 |
|---|---|---|
| SNIP1 | BioGRID | 1 |
| CDKL5 | BioGRID | 1 |
| EWSR1 | BioGRID | 1 |
| XPO1 | BioGRID | 1 |
| CTBP2 | BioGRID | 1 |
| THOC2 | BioGRID | 1 |
| PPIL1 | BioGRID | 1 |
| ZNF786 | BioGRID | 1 |


### TE 调控评估

该蛋白具有核定位证据，可能间接参与 TE 调控。需实验验证。

### HPA IF 图像

HPA 检索: https://www.proteinatlas.org/search/GPALPP1

### PubMed

**Count: 2**

| PMID | Title |
|---|---|
| 38522031 | The temporal protein signature analyses of developing human deciduous molar tooth germ. |
| 37330465 | Construction and verification of atopic dermatitis diagnostic model based on pyroptosis related biological markers using machine learning methods. |


