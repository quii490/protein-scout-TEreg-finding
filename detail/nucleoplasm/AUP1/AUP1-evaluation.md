...
**评价**: HPA 主定位为 Nucleoplasm, Vesicles。标注定位包括: Nucleoplasm, Vesicles。抗体可靠性评级: Approved。HPA IF 原图未可靠获取（HPA检索页无可用的subcellular IF原图）。核定位基于HPA localization/reliability + UniProt + GO-CC。
