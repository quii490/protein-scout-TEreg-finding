...
SPATS2L（SPATS2-like protein, 558 aa, UniProt Q9NUQ6）。定位于Nucleoli/Cytosol（HPA Enhanced），UniProt注释包含Nucleus, nucleolus。InterPro注释IPR009816和IPR009060（SPATS2-like家族），Pfam PF07139（SPATSL domain）。AlphaFold pLDDT=63.1（有序区35.7%），无PDB实验结构。

从PPI网络角度，STRING预测伙伴呈现干扰素应答特征——RTP4（0.613）、EPSTI1（0.584）、OAS3（0.578）、IFI44L/IFI44（0.554）、IFI16（0.547）、DDX60（0.540）、RSAD2（0.512）、IFI27（0.511）——均为经典I型干扰素刺激基因（ISG），提示SPATS2L可能在抗病毒免疫中发挥作用。这一推断得到IntAct数据支持：CSNK2A1（casein kinase 2 alpha 1）的实验互作——CK2磷酸化SPATS2L，可能与干扰素信号转导有关（PMID:22113938）。BioGRID额外鉴定HNRNPU和MCM2互作，后者提示可能的DNA复制连接。

从功能机制角度，文献报道SPATS2L是I型干扰素信号通路的正反馈调控因子，在狼疮中发挥关键作用（PMID:39099414）。在nucleoli中，SPATS2L可能与核仁应激应答和核糖体生物合成调控相关——IFI16（score=0.547, experimental=0.510）是核内DNA传感器和转录调控因子，与SPATS2L在nucleoli中共存。

从TE调控角度，I型干扰素通路的激活与内源性逆转录病毒的转录去抑制密切相关——ERV的LTR启动子可在干扰素刺激下激活，而IFI16已作为细胞质内逆转录病毒DNA传感器被研究。SPATS2L作为ISG蛋白，若正向调控干扰素信号，可能间接影响TE表达。此外，在狼疮中，抗RNP自身抗体和IFN信号常伴随TE异常表达。综合评分66.7/100。建议在干扰素刺激下检测SPATS2L是否重新分布至ERV位点。
...
| InterPro | IPR009816;IPR009060; |
| Pfam | PF07139; |

### humanPPI / HPA Interaction
Source: https://www.proteinatlas.org/ENSG00000196141-SPATS2L/interaction
