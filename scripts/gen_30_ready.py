#!/usr/bin/env python3
"""Generate full /180 re-evaluation reports for 30 ready genes.
...
GENES = [
    "CAPN11", "CAPNS2", "CARHSP1", "CARMIL2", "CARMIL3", "CBFA2T3",
    "CBLL2", "CBWD3", "CBWD5", "CBWD6", "CBX3P9", "CBX7",
    "CCDC9", "CCDC90B", "CCNDBP1", "CCNK", "CCNQ", "CCP110",
    "CD300C", "CDCA7", "CDK2AP2", "CDK5RAP2", "CDKL3", "CDKL4",
    "CDKN2AIPNL", "CDNF", "CDRT15L2", "CELF5", "CENPBD1", "CEP128",
]
    ...
    for c in comments:
        if c.get("commentType") == "INTERACTION":
            for inter in c.get("interactions", []):
                p1 = inter.get("interactantOne", {}).get("uniProtKBCrossReferences", [])
                p2 = inter.get("interactantTwo", {}).get("uniProtKBCrossReferences", [])
                partners = []
                if p1:
                    partners.append(p1[0].get("id", ""))
                if p2:
                    partners.append(p2[0].get("id", ""))
...
def fetch_alphafold(accession):
    """Fetch AlphaFold data using v2/3 API."""
    ...
    # New AF API uses fraction-based fields
    global_metric = entry.get("globalMetricValue", 0)
    mean_plddt = round(global_metric, 1) if global_metric else 0

    # Fractions (0-1 range)
    f_very_low = entry.get("fractionPlddtVeryLow", 0) or 0  # <50
    f_low = entry.get("fractionPlddtLow", 0) or 0            # 50-70
    f_confident = entry.get("fractionPlddtConfident", 0) or 0  # 70-90
    f_very_high = entry.get("fractionPlddtVeryHigh", 0) or 0   # >90
...
def fetch_pubmed(gene):
    """Fetch PubMed strict count via E-utils."""
    # Strict: gene[Title/Abstract] AND human
...
def fetch_hpa(gene):
    """Fetch HPA subcellular localization data."""
    # Try search endpoint
    ...
    for item in data:
        if item.get("Gene") and item.get("Gene", "").upper() == gene.upper():
            ...
            break
    if not entry:
        # Try first result if gene name matches
        for item in data:
    ...
    # Normalize to strings (API may return list or string)
    ...
    hpa_gene_url = f"https://www.proteinatlas.org/{ensembl}-{gene}" if ensembl else f"https://www.proteinatlas.org/search/{gene}"
    ...
    # Determine IF image availability
...
def fetch_intact(gene):
    """Fetch IntAct experimental interactions."""
...
def get_domain_text(uni_data):
    ...
    ipr_ids = [i.get("id", "") for i in interpro if i.get("id")]
    pf_ids = [p.get("id", "") for p in pfam if p.get("id")]
    # Also check feature domains
...
def download_pae_image(gene, accession, category):
    """Download PAE plot from AlphaFold."""
...
def already_complete(gene):
    """Check if gene already has a FULL report."""
...
def generate_report_for_gene(gene):
    """Fetch all data and generate a full report."""
    print(f"  [{gene}] Fetching data...", end=" ", flush=True)
...
def main():
    ...
    for i, gene in enumerate(to_process):
        try:
            print(f"[{i+1}/{len(to_process)}] {gene}")
            ...
            status_tag = "REJECTED" if rejected else "ACCEPTED"
