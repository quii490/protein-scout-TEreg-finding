#!/usr/bin/env python3
"""Generate complete /180 re-evaluation reports for 80 genes from harvest packets.
Downloads and embeds IF images where available. Skip already complete. No placeholders."""
...
ALL_GENES = [
    "C6orf52", "C6orf89", "C7orf25", "C8orf34", "C8orf48", "C8orf58", "C8orf76",
    "CABCOCO1", "CACNA1D", "CACUL1", "CADPS2", "CAMK2N2", "CAMKK1", "CAMLG",
    "CAND2", "CAPN11", "CAPNS2", "CARHSP1", "CARMIL2", "CARMIL3", "CBARP",
    "CBFA2T2", "CBFA2T3", "CBLL2", "CBWD3", "CBWD5", "CBWD6", "CBX3P9", "CBX7",
    "CCDC13", "CCDC83", "CCDC9", "CCDC90B", "CCHCR1", "CCNDBP1", "CCNK", "CCNQ",
    "CCP110", "CD300C", "CDCA7", "CDK2AP2", "CDK5RAP2", "CDKL3", "CDKL4",
    "CDKN2AIPNL", "CDNF", "CDRT15L2", "CDV3", "CELF5", "CENPBD1", "CEP112",
    "CEP128", "CEP131", "CEP250", "CEP350", "CEP55", "CEP76", "CEP78", "CEP85L",
    "CFAP119", "CFAP206", "CFAP47", "CFAP61", "CFAP97D2", "CFDP1", "CGGBP1",
    "CHAF1A", "CHCHD6", "CHD1L", "CHFR", "CHID1", "CHMP1B", "CIBAR2", "CIDEA", "CIMIP3",
]
...
# --- Report already complete check ---

def already_complete(gene):
    """Check if gene already has a FULL report (not thin placeholder)."""
...
def generate_report(gene):
    ...
    if image_status == "if_display_images_available":
        if_embed, if_note_raw = fetch_if_images_for_gene(gene, "rejected")
    ...
    if struct >= 10:
        if plddt >= 85:
            ...
            struct_desc = f"PDB实验结构（{pdb_text[:60]}）+ AlphaFold预测（pLDDT={round(plddt,1)}），实验结构可用。"
    elif struct >= 9:
...
def main():
    ...
    print(f"Total unique genes: {len(genes_unique)}")
    print("Checking for genes already complete and genes with harvest packets...\n")

    to_process = []
    for gene in genes_unique:
        packet_path = os.path.join(PACKET_DIR, f"{gene}.json")
        has_packet = os.path.exists(packet_path)
        ...
        to_process.append((gene, has_packet))

    has_pkt = sum(1 for _, p in to_process if p)
    no_pkt = sum(1 for _, p in to_process if not p)
    print(f"\nTo process: {len(to_process)} genes ({has_pkt} with packets, {no_pkt} without)")
    print("=" * 70)

    results = []
    for i, (gene, has_packet) in enumerate(to_process):
        try:
            print(f"\n[{i+1}/{len(to_process)}] {gene}...", end=" ", flush=True)
            ...
            results.append({"gene": gene, "dest": dest, "size": size, "status": status, "has_packet": has_packet})
        except Exception as e:
            print(f"-> ERROR: {e}")
            import traceback
            traceback.print_exc()
            results.append({"gene": gene, "dest": "ERROR", "size": 0, "status": f"ERROR: {e}", "has_packet": has_packet})
    ...
    no_pkt = sum(1 for r in results if r["status"] == "NO_PKT")
    err = sum(1 for r in results if r["status"].startswith("ERROR"))
    print(f"Summary: {ok} OK, {short} SHORT, {no_180} NO_180, {no_pkt} NO_PKT, {err} ERROR")
    dests = sorted(set(r["dest"] for r in results))
