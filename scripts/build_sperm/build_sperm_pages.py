#!/usr/bin/env python3
"""
Build HTML pages for the sperm module from Markdown evaluation reports.

Outputs:
    docs/sperm/index.html
    docs/sperm/protein_index.html
    docs/sperm/reports/GENE.html (for each gene)
"""
import json
import os
import re
...
def build_report_pages():
    ...
    detail_dir = "sperm/detail"
    os.makedirs("docs/sperm/reports", exist_ok=True)

    count = 0
    for gene_dir in sorted(os.listdir(detail_dir)):
        md_path = os.path.join(detail_dir, gene_dir, f"{gene_dir}-sperm-evaluation.md")
...
<title>{gene_dir} — Sperm Module | Protein Scout</title>
<style>
:root {{
  --bg: #fafafa;
  --fg: #1a1a1a;
...
<body>
<div class="topbar">
    <strong>🧬 Sperm Module — {gene_dir}</strong>
    <div>
        <a href="../index.html">Sperm Home</a>
...
{html_body}
<div class="back-link">
    <a href="../protein_index.html">← Back to Sperm Index</a>
</div>
</div>
</body>
</html>"""

        out_path = f"docs/sperm/reports/{gene_dir}.html"
...
def build_index_page():
    """Build the sperm module home page."""
    with open("sperm/data/sperm_report_index.json") as f:
        index = json.load(f)
    ...
    # Top 10 by score
    top10 = sorted(records, key=lambda r: r.get('final_sperm_score', 0), reverse=True)[:10]
    ...
    top_rows = '\n'.join(
        ...
        f'<td>{r.get("final_sperm_score", "-")}</td></tr>'
        for r in top10
    )
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Sperm Module | Protein Scout</title>
<style>
:root {{
...
<div class="container">
    <h1>🧬 Sperm Module</h1>
    <div class="note">
        <strong>Independent Extension</strong> — This sperm module is a separate
        evaluation pipeline from the main protein-scout TEreg-finding atlas.
        Sperm proteins are evaluated using sperm-specific criteria.
    ...
    <div class="stats">
        ...
        <div class="stat-card">
            ...
            <div class="label">SPERM_CANDIDATE</div>
        </div>
        <div class="stat-card">
    ...
    <tr><td>精子证据</td><td>×4</td><td>0–20</td><td>HPA多部位(18) / 精子(16) / 卫星(14)</td></tr>
    ...
    <p style="margin-top:8px;"><strong>公式:</strong> (精子×4 + 文献×4 + PPI×3 + 结构×2 + 新颖×2) / 2.6 → 0–100</p>
    <p><strong>淘汰:</strong> PubMed > 100 → ELIMINATED。不使用核定位评分淘汰。</p>
    </div>

    <p style="margin-top:24px; color: var(--muted);">
        <a href="protein_index.html">→ View full sperm protein index</a>
    </p>
    ...
    with open("docs/sperm/index.html", 'w') as f:
        f.write(html)
    print("  Built docs/sperm/index.html")


def build_protein_index():
    """Build the full sperm protein table page with separated candidate/eliminated tables."""
    with open("sperm/data/sperm_report_index.json") as f:
        index = json.load(f)

    records = index['records']
    candidates = sorted([r for r in records if 'eliminated' not in r.get('status','').lower()],
                       key=lambda r: r.get('final_sperm_score', 0), reverse=True)
    ...
    cand_rows = '\n'.join(
        ...
        f'<td class="score-cell">{r.get("final_sperm_score", "-")}</td>'
        f'<td>{r.get("sperm_evidence_score", "-")}</td>'
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>精子蛋白索引 | Protein Scout</title>
<style>
:root {{
  --bg: #fafafa; --fg: #1a1a1a; --accent: #7c3aed;
  --accent-light: #ede9fe; --border: #e5e7eb; --muted: #6b7280; --card: #ffffff;
...
<div class="container">
    ...
    <table id="candidateTable">
        <thead><tr>
            ...
            <th>精子证据</th><th>PubMed</th><th>新颖性</th>
    ...
    <div class="note">⚠️ ELIMINATED 仅表示 PubMed&gt;100 或不适合当前精子候选优先级，不代表生物学无关。</div>
    <table id="eliminatedTable">
        <thead><tr>
            <th>Gene</th><th>蛋白全名</th><th>状态</th><th>PubMed</th>
            <th>淘汰原因</th><th>精子证据</th><th>酶</th>
        </tr></thead>
...
function filterCurrentTable() {{
    ...
    with open("docs/sperm/protein_index.html", 'w') as f:
        f.write(html)
    print("  Built docs/sperm/protein_index.html")


def main():
    print("Building sperm HTML pages...")
    build_report_pages()
    build_index_page()
    build_protein_index()
    print("Done.")
