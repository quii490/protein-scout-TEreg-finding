#!/usr/bin/env python3
"""
Build sperm report index from sperm/detail/ evaluation reports.

Outputs:
    sperm/data/sperm_report_index.tsv
    sperm/data/sperm_report_index.csv
    sperm/data/sperm_report_index.json
    docs/sperm/data/sperm_report_index.json
"""
import json
import os
import sys
...
def parse_evaluation(path):
    """Parse a sperm evaluation markdown report."""
    with open(path) as f:
        content = f.read()
    ...
    # Source info
    source_centro = 'sperm' in frontmatter.get('tags', '') or 'Sperm' in content[:500]
    ...
    return {
        ...
        'main_category': '',
        'report_path': path,
        'docs_report_path': f"docs/sperm/reports/{gene}.html",
...
def main():
    ...
    for gene_dir in sorted(os.listdir(base)):
        ...
        if not os.path.isdir(dir_path):
            continue
        md_path = os.path.join(dir_path, f"{gene_dir}-sperm-evaluation.md")
        if os.path.exists(md_path):
            try:
                report = parse_evaluation(md_path)
                reports.append(report)
                print(f"  Parsed: {gene_dir} → score={report['final_sperm_score']}")
    ...
    reports.sort(key=lambda r: r['final_sperm_score'], reverse=True)

    # Write TSV
    fields = [
        'gene', 'protein_full_name', 'uniprot_id', 'status', 'final_sperm_score',
        'sperm_evidence_score', 'pubmed_score', 'ppi_score',
        'structure_domain_score', 'novelty_specificity_score',
        'pubmed_strict_count', 'pubmed_broad_count', 'pubmed_display_count',
        'source_sperm', 'source_centriolar_satellite', 'source_both',
    ...
    os.makedirs("sperm/data", exist_ok=True)
    os.makedirs("docs/sperm/data", exist_ok=True)

    # TSV
    with open("sperm/data/sperm_report_index.tsv", 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fields, delimiter='\t')
        writer.writeheader()
        writer.writerows(reports)
    print(f"  TSV: sperm/data/sperm_report_index.tsv ({len(reports)} records)")

    # CSV
    with open("sperm/data/sperm_report_index.csv", 'w', newline='') as f:
        writer = csv.DictWriter(f, fieldnames=fields)
        writer.writeheader()
        writer.writerows(reports)
    print(f"  CSV: sperm/data/sperm_report_index.csv ({len(reports)} records)")
    ...
    pubmed_data = {}
    prescreen_path = "sperm/data/pubmed_prescreen_results.json"
    ...
    for r in reports:
        gene = r['gene']
        md_path = f"sperm/detail/{gene}/{gene}-sperm-evaluation.md"
        if os.path.exists(md_path):
            c = open(md_path).read()
    ...
    with open("sperm/data/sperm_report_index.json", 'w') as f:
        f.write(json_str)
    with open("docs/sperm/data/sperm_report_index.json", 'w') as f:
        f.write(json_str)
    print(f"  JSON: sperm/data/sperm_report_index.json ({len(reports)} records)")
    print(f"  JSON: docs/sperm/data/sperm_report_index.json ({len(reports)} records)")
