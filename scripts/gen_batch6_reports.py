#!/usr/bin/env python3
"""Generate 5 full re-evaluation reports for Batch 6 recovery from false-rejection.
Genes: MS4A12 MSANTD1 MSANTD3 MSANTD4 MSRB1"""
import json, os, sys
...
# Map gene -> destination subdirectory under detail/
GENE_DEST = {
    "MS4A12": "nucleus-cytoplasm",
    "MSANTD1": "nuclear-body",
    "MSANTD3": "nucleus-cytoplasm",
    "MSANTD4": "nucleoplasm",
    "MSRB1": "nucleoplasm",
}
...
def domain_score(domains_text, plddt, prot_name=""):
    ...
    # Also check protein name for SANT/MYB keywords
    name_lower = prot_name.lower()
    has_chromatin_text = any(kw in text for kw in chromatin_kw)
    has_chromatin_name = any(kw in name_lower for kw in chromatin_kw)
    has_chromatin = has_chromatin_text or has_chromatin_name
...
def format_intact_table(partners, gene):
    ...
    for p in partners:
        ...
        method = p.get("method", "—")
        pmid = p.get("pmid", "—")
        lines.append(f"| {partner} | {method[:50]} | {pmid[:40]} |")
...
def generate_report(gene):
    ...
    if has_uni:
        ...
        # Use HPA-derived info
        hpa_uniprot = hpa.get("uniprot", ["unknown"])
        accession = hpa_uniprot[0] if hpa_uniprot else gene
        prot_name = f"{gene} (UniProt获取失败)"
        length_aa = 0
        mass_kda = 0
        subcell = []
        go_cc = []
    ...
    dom = domain_score(domain_text, plddt, prot_name)
    ...
    dest = GENE_DEST.get(gene, "rejected")
    ...
    elif nov >= 4: nov_desc = "已有一定研究基础，但仍有niche空间。"
    else: nov_desc = "研究较多，novelty价值有限。"
    ...
    if struct >= 10:
        ...
        struct_desc = f"AlphaFold 预测质量一般（pLDDT={plddt}，有序区 {ordered_pct}%），部分区域可信度有限。"
    else:
    ...
    if dom >= 9:
        ...
        dom_desc = f"多个已知结构域注释，{prot_name[:40]}具备潜在的染色质/DNA相互作用能力。"
    elif dom >= 7:
    ...
    uniprot_url = f"https://www.uniprot.org/uniprotkb/{accession}" if accession and has_uni else (f"https://www.uniprot.org/uniprotkb/{accession}" if accession else "")
    ...
    if rejected:
        title_line = f"## {gene} — REJECTED (核定位 {nuc}/10 ≤ 3)"
    else:
        title_line = f"## {gene} 核蛋白评估报告 (Full Re-evaluation)"

    # Build verbose domain analysis based on actual domain content
    domain_verbose = ""
    if "SANT" in prot_name or "Myb" in prot_name or "DNA-binding" in prot_name:
        domain_verbose = (
            f"\n\n**关键发现**: 蛋白名称中明确标注\"Myb/SANT-like DNA-binding domain\"，SANT结构域是"
            f"染色质重塑复合体中识别组蛋白尾部的关键模块，与MYB相关的DNA结合功能密切相关。"
            f"该结构域注释表明{gene}可能通过DNA结合参与转录调控或染色质修饰过程。"
        )
    elif "暂无" in domain_text:
        domain_verbose = "\n\n**注意**: UniProt数据获取失败，结构域注释不可用。"
    else:
        domain_verbose = ""

    # For MSRB1, add special note about selenoprotein
    seleno_note = ""
    if gene == "MSRB1":
        seleno_note = (
            "\n\n**特殊注释**: MSRB1 是一种含硒半胱氨酸 (selenocysteine) 的硒蛋白，"
            "属于甲硫氨酸亚砜还原酶 (Msr) 家族。其酶活性中心含有Sec (U) 残基，"
            "负责还原蛋白中的甲硫氨酸-R-亚砜，在抗氧化防御和氧化还原调控中发挥关键作用。"
            "作为硒蛋白，其表达受硒元素摄入水平调控。"
        )
...
gene: "{gene}"
date: 2026-06-03
tags: [protein-scout, {"rejected" if rejected else "nuclear-protein"}, evaluation, recovery-from-false-rejection]
status: {status}
---
...
| UniProt ID | {accession if accession else "未知"} |
| 评估日期 | 2026-06-03 |
...
| 来源 | 结构域 |
|------|--------|
| InterPro/Pfam | {domain_text[:150]} |{domain_verbose}
...
2. {"AlphaFold 预测质量一般（pLDDT=" + str(plddt) + "），需要更多实验结构验证" if plddt < 70 and af_found else "结构数据质量可接受" if plddt >= 70 and af_found else "结构数据暂缺（AlphaFold未找到或UniProt获取失败）" if not af_found else "结构数据质量可接受"}

**下一步建议**:
- [ ] 查阅最新关键文献补充研究背景
- [ ] 获取 Protein Atlas IF 图像确认亚细胞定位
- [ ] 设计体外实验验证核定位及潜在调控功能
{"- [ ] **该蛋白核定位证据不足（≤3/10），不建议作为核蛋白研究目标。**" if rejected else ""}
...
- AlphaFold: {"https://alphafold.ebi.ac.uk/entry/" + accession if accession and af_found else "N/A (未找到预测结构)"}
...
print("Generating 5 Batch 6 re-evaluation reports...")
for gene in ["MS4A12", "MSANTD1", "MSANTD3", "MSANTD4", "MSRB1"]:
    dest_dir = GENE_DEST.get(gene, "rejected")
    report_dir = os.path.join(DETAIL, dest_dir, gene)
    ...
    try:
        ...
        status_tag = "OK" if size > 3000 else "SHORT"
        print(f"  {gene} -> {dest_dir}/{gene}/{gene}-evaluation.md ({size} chars) [{status_tag}]")
