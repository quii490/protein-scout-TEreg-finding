#!/usr/bin/env python3
"""Generate full /180 reports for all 113 ready genes.
Uses harvest packets where available (fast), live API fetch where not.
...
GENES = [
    "CARD8","CARF","CBLC","CHCHD1","CHMP2A","CHMP4B","CHN2","CHPF2","CHST1","CHST2","CHST7",
    "CHTF18","CIAO1","CIAO2A","CIAO2B","CIAPIN1","CIB1","CIMAP1D","CINP","CIRSR","CLCN4",
    "CLEC18B","CLEC2B","CLGN","CLIC5","CLIP2","CLN5","CLPB2","CLP2","CLPTM1L","CLRN1",
    "CLRN2","CLSTN2","CLUAP1","CLVS1","CMBL","CLYBL","CMC1","CNDP1","CNFN1","CNNM1",
    "CNNM3","CNOT1","CNOT10","CNOT2","CNP","CNPY2","CNPY4","CNST","COA4","COG1","COG2",
    "COG8","COMMD4","COMMD5","COPS4","COPS5","COPS6","COQ2","COQ3","COQ4","COQ5",
    "COQ6","CORO1A","COTL1","CPA4","CPAMD8","CPEB4","CPNE7","CPSF2","CPSF3","CPSF3L",
    "CPT1C","CPTP","CPVL","CRACR2B","CRBNP","CROT","CRYAA","CRYAB","CRYBB1","CRYGS",
    "CRYZ","CS","CRYBG1","CSGALNACT1","CSNK1G1","CTBP1","CTNNAL1","CTNND1","CTPS1",
    "CTU1","CTU2","CUL4B","CXADR","CXCL14","CXXC1","CXorf21","CYB5D1","CYB5D2",
    "CYP20A1","CYP27C1","CYP2S1","CYP39A1","CYP4F22","CYP4V2","CYP4X1","CYP4Z1",
]
...
def load_from_packet(gene):
    """Load pre-fetched data from harvest packet JSON. Returns tuple of same format as live fetchers."""
...
# ============================================================
# LIVE API FETCHERS (fallback when no harvest packet)
# ============================================================
...
def fetch_uniprot_live(gene):
    ...
    return {
        ...
        "function": functions,
        "subcellular_locations": [{"location": s, "evidences": []} for s in subcell] if subcell else [{"location": s, "evidences": []} for s in subcell],
        "go_cc": go_cc,
        "pdb": pdb,
        "interpro": interpro,
        "pfam": [],
...
def generate_report_for_gene(gene, use_packet=False):
    ...
    # IF images - skip for speed unless packet has image_urls
    # PAE - skip for speed
    if_embed = None
    if_note = ""
