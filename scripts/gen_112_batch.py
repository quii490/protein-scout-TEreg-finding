#!/usr/bin/env python3
"""Generate full /180 reports for 112 ready genes (CNDP2..DNAJC8 batch).
SKIP if already complete. Uses harvest packets where available, live API fetch where not."""
...
GENES = [
    "CNDP2","CNEP1R1","CNTD1","CNTNAP1","CNTNAP5","COG3","COG6","COPG1","COPRS","COQ7",
    "CPED1","CPNE1","CPNE2","CRADD","CRCP","CREBZF","CRIP3","CRYL1","CRYZL1","CTNNBIP1",
    "CTPS2","CUEDC2","CWF19L1","CWF19L2","CYB5R2","CYRIA","CYTIP","CYYR1","Chtf8","Crxos",
    "DACH1","DAGLA","DAND5","DAZAP1","DBF4B","DBNDD1","DCAF11","DCAF4L2","DCAF6","DCTN2",
    "DCTN5","DDAH2","DDX19A","DDX31","DDX39A","DDX42","DDX46","DDX51","DDX55","DDX56",
    "DELE1","DENND10","DENND11","DENND4B","DENND5B","DEPDC1B","DERA","DET1","DGCR14","DGKA",
    "DHRS1","DHRS12","DHRS13","DHRS7C","DHX29","DHX30","DHX33","DHX34","DHX37","DHX38",
    "DHX58","DHX9","DIAPH1","DIDO1","DIMT1","DIS3","DKC1","DLAT","DLG4","DMPK",
    "DMTF1","DMXL1","DMXL2","DNA2","DNAAF3","DNAAF5","DNAAF6","DNAJA2","DNAJA3","DNAJB1",
    "DNAJB11","DNAJB12","DNAJB13","DNAJB14","DNAJB2","DNAJB5","DNAJB6","DNAJB8","DNAJB9",
    "DNAJC10","DNAJC11","DNAJC14","DNAJC18","DNAJC19","DNAJC2","DNAJC21","DNAJC28","DNAJC3",
...
def load_from_packet(gene):
    ...
    af_result = {"found": False} if not af_found else {
        ...
        "pae_image_url": af_data.get("pae_image_url", ""),
        "pae_doc_url": af_data.get("pae_doc_url", ""),
        "plddt_url": af_data.get("plddt_url", ""),
    ...
    hpa_result = {
        "ok": hpa_ok,
        "data": {
            "checked_urls": hpa_data.get("checked_urls", []),
            "status": hpa_data.get("status", "unknown"),
            "gene": gene,
            "ensembl": hpa_data.get("ensembl", ""),
            "uniprot": hpa_data.get("uniprot", []),
            ...
            "thumbnail_urls": hpa_data.get("thumbnail_urls", []),
            "medium_image_urls": hpa_data.get("medium_image_urls", []),
            "if_image_urls": hpa_data.get("if_image_urls", []),
            "image_urls": hpa_data.get("image_urls", []),
            "image_status": hpa_data.get("image_status", "unknown"),
        }
    }

    str_raw = pkt.get("string", {})
    str_ok = str_raw.get("ok", False)
    string_result = str_raw.get("data", []) if str_ok else []

    inta_raw = pkt.get("intact", {})
    inta_ok = inta_raw.get("ok", False)
    intact_result = inta_raw.get("data", []) if inta_ok else []
...
def fetch_uniprot_live(gene):
    ...
    return {
        ...
        "function": functions,
        "subcellular_locations": [{"location": s, "evidences": []} for s in subcell] if subcell else [],
...
def fetch_alphafold_live(accession):
    ...
    lt50 = round(f_vl * 100, 1)
    r_50_70 = round(f_l * 100, 1)
    r_70_90 = round(f_c * 100, 1)
    gt90 = round(f_vh * 100, 1)
    ...
    return {
        ...
        "plddt_stats": {
            ...
            "pct_gt_90": gt90, "pct_70_90": r_70_90,
            "pct_50_70": r_50_70, "pct_lt_50": lt50,
        }
    }
...
def fetch_pubmed_live(gene):
    ...
    return {
        "strict_query": query, "strict_count": strict_count,
        "symbol_only_query": gene, "symbol_only_count": broad_count,
        "broad_query": gene, "broad_count": broad_count,
        "alias_note": "", "key_papers": key_papers,
    }
...
def fetch_hpa_live(gene):
    ...
    return {
        ...
        "data": {
            ...
            "thumbnail_urls": [], "medium_image_urls": [],
            "if_image_urls": [], "image_urls": [],
...
def nuclear_score(uni_subcell, hpa_main, hpa_addl, hpa_rel, go_cc_terms, has_uni):
    ...
    # Average subcellular_locations (from packet they're dicts, from live they might be dicts or strings)
    subcell_vals = []
    for s in uni_subcell:
        if isinstance(s, dict):
            subcell_vals.append(s.get("location", ""))
        else:
            subcell_vals.append(str(s))

    hpa_nuc = False; hpa_non = False
    ...
    uni_nuc = False; uni_non = False
    if has_uni:
        uni_text = " ".join(subcell_vals).lower()
        if any(kw in uni_text for kw in nucleus_kw):
            uni_nuc = True
        if any(kw in uni_text for kw in non_nuc_kw):
            uni_non = True

    go_nuc = False; go_non = False
    if go_cc_terms:
        go_terms = []
...
def extract_go_terms(go_cc):
    ...
    terms = []
    for cc in go_cc:
        if isinstance(cc, dict): terms.append(cc.get("term", str(cc)))
        else: terms.append(str(cc))
    return terms
...
def determine_dest(nuc, pubmed_strict, hpa_main, hpa_addl, subcell, go_cc):
    ...
    subcell_vals = []
    for s in subcell:
        if isinstance(s, dict): subcell_vals.append(s.get("location", ""))
        else: subcell_vals.append(str(s))
    uni_loc_text = " ".join(subcell_vals).lower()
...
def generate_report_for_gene(gene, use_packet=False):
    if use_packet:
    ...
    if has_uni:
        ...
        for p in uni.get("pdb", []):
            if isinstance(p, dict): pdb_list.append(p.get("id", ""))
            else: pdb_list.append(str(p))
        aliases = uni.get("aliases", [])
    else:
        accession = gene; prot_name = f"{gene} (UniProt未获取)"
        length_aa = 0; mass_kda = 0; subcell = []; go_cc = []
        function_text = []; pdb_list = []; aliases = []
    ...
    if af_found:
        ...
        plddt = 0; pct_gt_90 = 0; pct_70_90 = 0; pct_50_70 = 0
        pct_lt_50 = 0; af_version = "?"; ordered_pct = 0
    ...
    if dom >= 9: dom_desc = "含明确的核酸结合/染色质相关结构域，多库确认。"
    elif dom >= 8: dom_desc = "多个已知结构域注释，AlphaFold预测质量高，结构域折叠可信。"
    elif dom >= 7: dom_desc = "存在已知结构域注释，可作为功能研究的结构基础。"
    elif dom >= 5: dom_desc = "结构域注释有限，AlphaFold预测有可辨识折叠域。"
    else: dom_desc = "结构域注释稀疏，属新颖蛋白正常现象。"
...
def main():
    ...
    for gene in GENES:
        ...
        if os.path.exists(pkt_path):
            ...
            to_process_live.append(gene)

    print(f"\n{'='*70}")
    print(f"Total genes: {len(GENES)}")
    print(f"  Already complete (skip): {len(skipped)}")
    print(f"  From harvest packets:    {len(to_process)}")
    print(f"  Live API fetch needed:   {len(to_process_live)}")
    print(f"{'='*70}\n")

    results = []

    # Phase 1: Process harvest packet genes (fast, no API calls)
    if to_process:
        print(f"=== PHASE 1: Processing {len(to_process)} genes from harvest packets ===\n")
        for i, gene in enumerate(to_process):
            try:
                print(f"[{i+1}/{len(to_process)}] {gene}", end="", flush=True)
                report, dest, rejected = generate_report_for_gene(gene, use_packet=True)
        ...
        print(f"\nPhase 1 complete: {len(results)} processed.\n")

    # Phase 2: Live API fetch for remaining genes
    if to_process_live:
        print(f"=== PHASE 2: Processing {len(to_process_live)} genes with live API ===\n")
        for i, gene in enumerate(to_process_live):
            try:
                print(f"[{i+1}/{len(to_process_live)}] {gene}", end="", flush=True)
                report, dest, rejected = generate_report_for_gene(gene, use_packet=False)
