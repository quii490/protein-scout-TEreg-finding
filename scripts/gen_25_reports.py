...
GENES = [
    "ARL14EP", "ARL2BP", "ARL6", "ARMC8", "ARMCX4", "ARMCX5", "ASAP3", "ASCL3", "ASCL5",
    "ASZ1", "ATG10", "ATG2A", "ATOX1", "ATXN3L", "ATXN7L3B", "B4GALNT4", "BACC1", "BCL7C",
    "BCLAF3", "BEAN1", "BEGAIN", "BEND4", "BEND5", "BET1L", "BEX4"
]
...
def nuclear_score(hpa_main, hpa_all, uniprot_locs, go_cc_terms):
    ...
    REJECTED if score <= 3."""
    # Collect nuclear evidence
    nuclear_signals = []

    # HPA main locations (strongest signal)
    if hpa_main:
        for loc in hpa_main:
            loc_lower = loc.lower()
...
def classify_category(hpa_main, hpa_all, uniprot_locs, nuc_score):
    """Determine best category based on localization data."""
    if nuc_score <= 3:
        return "rejected"
    ...
    if ('nucleoplasm' in all_text or 'nucleus' in all_text):
        # Check for cytoplasmic co-localization
...
def build_report(gene, pkt):
    ...
    if if_image_urls and len(if_image_urls) > 0:
        lines.append("")
        lines.append("**IF 图像**:")
        detail_dir = f"detail/{category}/{gene}" if not rejected else f"detail/rejected/{gene}"
    ...
    if af_available and isinstance(plddt_stats, dict) and plddt_stats and 'mean_plddt' in plddt_stats and plddt_stats.get('mean_plddt', 0):
        ...
        lines.append("| 指标 | 数值 |")
        lines.append("|------|------|")
        lines.append(f"| AlphaFold 版本 | v6 |")
...
def main():
    for gene in GENES:
        ...
        try:
            ...
            print(f"  -> {out_path} ({len(report)} chars) [category={category}, rejected={rejected}]")
