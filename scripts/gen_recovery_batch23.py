#!/usr/bin/env python3
"""Generate 10 full re-evaluation reports for Batch 2 & 3 recovery genes.
These were falsely-rejected due to missing harvest — now have fresh packets.

Batch 2: MLIP MLXIPL MMS19 MMS22L MNS1
Batch 3: MOB1A MOB1B MOB2 MOCS2 MON2
"""
import json, os, sys
...
GENES = [
    # Batch 2
    "MLIP", "MLXIPL", "MMS19", "MMS22L", "MNS1",
    # Batch 3
    "MOB1A", "MOB1B", "MOB2", "MOCS2", "MON2",
]
...
def generate_report(gene):
    ...
    pubmed_broad = pubmed.get("broad_count", 0)
    ...
    reject_reason = ""
    if nuc <= 3:
        reject_reason = f"核定位 {nuc}/10 ≤ 3"
    if pubmed_strict > 100:
        if reject_reason:
            reject_reason += f"; PubMed {pubmed_strict} > 100"
        else:
            reject_reason = f"PubMed {pubmed_strict} > 100"
    ...
    if rejected:
        ...
        # Determine appropriate subcellular category
        has_nucleolus = any("nucleol" in loc.lower() for loc in hpa_main + hpa_addl + subcell)
        if any("speckle" in loc.lower() for loc in hpa_main + hpa_addl):
            dest_cat = "nuclear-speckle"
        elif any("envelope" in loc.lower() for loc in hpa_main + hpa_addl + subcell):
            dest_cat = "nuclear-envelope"
        elif any("chromatin" in loc.lower() for loc in hpa_main + hpa_addl + subcell):
            dest_cat = "chromatin"
        elif any("nucleolus" in loc.lower() or "nucleoli" in loc.lower() for loc in hpa_main + hpa_addl + subcell):
            dest_cat = "nucleolus"
        elif any("body" in loc.lower() for loc in hpa_main + hpa_addl):
            dest_cat = "nuclear-body"
        elif "nucleoplasm" in " ".join(hpa_main + hpa_addl).lower():
            dest_cat = "nucleoplasm"
        elif "nucleus" in " ".join(subcell).lower() or "cytosol" in " ".join(hpa_main + hpa_addl).lower():
            dest_cat = "nucleus-cytoplasm"
        else:
            dest_cat = "nucleoplasm"
    ...
    nuc_desc_map = {
        ...
        7: "主要核定位，HPA 可靠性良好，有辅助数据源支持，部分数据源提示非核定位但核信号主导。",
    ...
    # HPA IF status - check for actual if_image_urls
    has_if = len(if_urls) > 0
    if not has_if and image_status not in ["if_display_images_available"]:
        if_note = "HPA IF 原图未可靠获取（HPA检索页无可用的subcellular IF原图）。核定位基于HPA localization/reliability + UniProt + GO-CC。"
    else:
        if_note = f"HPA IF 图像状态: {image_status}。"

    # IF image embed
    if_embed_text = ""
    if has_if:
        if_embed_text = "\n**HPA IF 图像**:\n"
        for idx, url in enumerate(if_urls[:3], 1):
            if_embed_text += f"![IF {idx}]({url})\n"
    ...
    uni_url = f"https://www.uniprot.org/uniprotkb/{accession}" if accession else ""

    nov_label = "≤20→10" if pubmed_strict <= 20 else ("≤40→8" if pubmed_strict <= 40 else ("≤60→6" if pubmed_strict <= 40 else ("≤80→4" if pubmed_strict <= 80 else "≤100→2")))

    # Fix nov_label
    if pubmed_strict <= 20: nov_label = "≤20→10"
    elif pubmed_strict <= 40: nov_label = "21-40→8"
    elif pubmed_strict <= 60: nov_label = "41-60→6"
    elif pubmed_strict <= 80: nov_label = "61-80→4"
    elif pubmed_strict <= 100: nov_label = "81-100→2"
    else: nov_label = ">100, rejected"
    ...
    else: cross_details.append(f"多库定位一致 ({has_multi_nuc}源): +0")
    ...
    # Function text
    func_desc = "; ".join(function_text) if function_text else "暂无功能注释。"
    func_desc = func_desc[:600] + ("..." if len(func_desc) > 600 else "")

    if rejected:
        title_line = f"## {gene} — REJECTED ({reject_reason})"
        rejection_extra = (
            f"\n> **拒因**: {reject_reason}。"
            f"\n> **数据来源**: HPA({', '.join(hpa_main[:3]) if hpa_main else '无'}), "
            f"UniProt({subcell_text[:60]}), GO-CC({', '.join(extract_go_terms(go_cc)[:3]) if go_cc else '无'})"
            f"\n> **PubMed (strict)**: {pubmed_strict}"
        )
    else:
        title_line = f"## {gene} 核蛋白评估报告 (Full Re-evaluation)"
        rejection_extra = ""
        recommended_label = "⭐" * max(1, min(5, round(norm/20)))

    report = f"""---
type: protein-evaluation
gene: "{gene}"
date: 2026-06-03
tags: [protein-scout, {"rejected-nuclear" if rejected else dest_cat.replace("-", "-")}, re-evaluation, recovery]
status: {status}
category: {dest_cat if not rejected else "rejected"}
...
| 维度 | 得分 | 满分 | 加权后 | 关键证据摘要 |
|------|------|------|--------|-------------|
| 🔴 核定位特异性 | {nuc}/10 | ×4 | {nuc*4} | HPA: {hpa_loc_text[:60] if hpa_loc_text else "暂无"}; UniProt: {subcell_text[:60]} |
| 📏 蛋白大小 | {sz}/10 | ×1 | {sz} | {length_aa if length_aa else "未知"} aa / {mass} kDa |
| 🆕 研究新颖性 | {nov}/10 | ×5 | {nov*5} | PubMed strict={pubmed_strict} 篇 ({nov_label}) |
| 🏗️ 三维结构 | {struct}/10 | ×3 | {struct*3} | AlphaFold v{af_version} pLDDT={plddt}，PDB: {pdb_text[:40]} |
...
| PubMed broad count | {pubmed_broad if pubmed_broad else pubmed.get("broad_count", "N/A")} |
...
**推荐等级**: {"⭐" * max(1, min(5, round(norm/20))) if not rejected else "REJECTED"}
...
**风险/不确定性**:
1. {"PubMed " + str(pubmed_strict) + " 篇，研究基础极有限，功能注释不完整" if pubmed_strict <= 20 else "PubMed " + str(pubmed_strict) + " 篇，已有一定研究基础"}
...
- [ ] 设计体外实验验证核定位及潜在调控功能
{"- [ ] **REJECTED: " + reject_reason + "，不建议作为核蛋白研究目标。**" if rejected else ""}

### 5. 数据来源
- UniProt: {uni_url}
- Protein Atlas: {hpa_gene_url if hpa_gene_url else "https://www.proteinatlas.org/search/" + gene}
- Protein Atlas Subcellular: {hpa_subcell_url if hpa_subcell_url else "N/A"}
...
*本报告基于最新的 harvest packet 数据（{pkt.get("timestamp", "N/A")}），各数据库实时抓取。评分严格遵循 /180 加权评分体系：核定位×4 + 大小×1 + 新颖性×5 + 结构×3 + 结构域×2 + PPI×3 + 互证加分（max+3）。PubMed>100 或 核定位≤3 为 REJECTED。*
"""
    return report, dest_cat


# Generate all reports
print("Generating 10 full re-evaluation reports (Batch 2 & 3 recovery)...")
results = []
for gene in GENES:
    try:
        report, dest_cat = generate_report(gene)
        report_dir = os.path.join(DETAIL, dest_cat, gene)
        os.makedirs(report_dir, exist_ok=True)
        report_path = os.path.join(report_dir, f"{gene}-evaluation.md")
        with open(report_path, "w") as f:
            f.write(report)
        size = len(report.encode("utf-8"))
        status = "OK" if size > 3000 else "SHORT"
        print(f"  {gene} -> {dest_cat}/{gene}/{gene}-evaluation.md ({size} chars) [{status}]")
        results.append({"gene": gene, "path": report_path, "size": size, "cat": dest_cat, "status": status})
...
print("\n--- Summary ---")
for r in results:
    print(f"  [{r['status']}] {r['gene']}: {r['cat']} ({r['size']} chars)")
print(f"\nDone. {len(results)}/10 reports generated.")
