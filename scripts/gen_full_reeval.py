#!/usr/bin/env python3
"""Generate 10 full re-evaluation reports for Batch 6 & 7 genes from harvest packets.
Replaces 11-line template rejected reports."""
import json, os, sys
...
GENES = [
    ...
    "ANAPC5", "ANAPC7", "ANGEL1", "ANGEL2", "ANKEF1",
]
...
def nuclear_score(uni_subcell, hpa_main, hpa_addl, hpa_rel, go_cc_terms, has_uni):
    ...
    if any(kw in hpa_main_text for kw in nucleolus_kw):
        hpa_nuc = True  # Nucleoli IS nuclear
    ...
    if any(kw in hpa_addl_text for kw in nucleus_kw) and not any(kw in hpa_main_text for kw in nucleus_kw):
        hpa_nuc = True  # Additional nuclear when main doesn't have it
    ...
    if nuc_sources == 0 and non_sources == 0:
        if has_uni:
            return 3  # UniProt exists but no location data
        return 3  # No data

    # Strongest case: multiple nuclear sources, no non-nuclear
    if nuc_sources >= 2 and non_sources == 0:
        return 9 if nuc_sources >= 3 else 8

    # Nuclear with some non-nuclear
    if nuc_sources >= 2 and non_sources >= 1:
        return 7

    # One nuclear source only
    if nuc_sources == 1 and non_sources == 0:
        if hpa_nuc and hpa_rel in ["Approved", "Enhanced"]:
            return 7  # HPA only but reliable
        return 5  # HPA only, unknown reliability

    # HPA (direct experimental) says ONLY non-nuclear (no nuclear signal at all)
    # but UniProt/GO says nuclear → genuine conflict
    # Check this BEFORE generic equal-sources to catch ANAPC13-like conflicts
    if hpa_non and not hpa_nuc and (uni_nuc or go_nuc):
        if go_nuc:
            return 4  # GO supports UniProt, conflicting with HPA
        return 3  # HPA only non-nuclear vs UniProt nuclear → weak nuclear

    # Nuclear with equal non-nuclear
    if nuc_sources == 1 and non_sources == 1:
        return 4  # Conflicting

    # Nuclear + more non-nuclear (mixed)
    if nuc_sources >= 1 and non_sources > nuc_sources:
        return 4

    # Only non-nuclear HPA evidence, conflicting with other sources
    if non_sources >= 2 and nuc_sources == 0:
        return 2

    # Only non-nuclear from one source
...
def extract_go_terms(go_cc):
    """Extract term strings from go_cc dict list."""
    if not go_cc: return []
    terms = []
    for cc in go_cc:
...
def generate_report(gene):
    ...
    sz = size_score(length_aa) if length_aa > 0 else 5  # default if unknown
...
print("Generating 10 full re-evaluation reports...")
for gene in GENES:
    ...
    try:
        ...
        status = "OK" if size > 2000 else "SHORT"
        print(f"  {gene} -> rejected/{gene}/{gene}-evaluation.md ({size} bytes) [{status}]")
