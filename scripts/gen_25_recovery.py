#!/usr/bin/env python3
"""Generate /180 re-evaluation reports for 25 recovery genes from harvest packets.
Recovery from false-rejection. Standard /180 scoring. Nuclear<=3->REJECTED. PubMed>100->REJECTED."""
import json, os
...
GENES = [
    "NEXMIF", "NFATC2IP", "NFKBIL1", "NFU1", "NGDN", "NHEJ1",
    "NHLRC1", "NHLRC2", "NHSL1", "NHSL3", "NIF3L1", "NIPAL3",
    "NKIRAS2", "NLE1", "NME7", "NME8", "NMRK1", "NMRK2",
    "NOC3L", "NOM1", "NOPCHAP1", "NOS1AP", "NPEPPS", "NPTXR", "NQO2"
]


# --- Scoring helpers (from gen_30_reeval.py) ---
...
def nuclear_score(uni_subcell, hpa_main, hpa_addl, hpa_rel, go_cc_terms, has_uni):
    ...
    # Also check additional locations for nuclear signals
...
def get_domain_text(uni_data):
    ...
    parts = []
    if ipr_ids:
        parts.append("InterPro: " + ", ".join(ipr_ids[:5]))
    if pf_ids:
        parts.append("Pfam: " + ", ".join(pf_ids[:3]))
...
def generate_report(gene):
    ...
    if not isinstance(string_p, list):
        string_p = []
    ...
    if sz >= 10:
        sz_desc = "大小适中（200-800 aa），适合常规生化实验和结构解析。"
    elif sz >= 8:
        sz_desc = "大小基本合适，可用于常规实验。"
    elif sz >= 5:
        sz_desc = "蛋白偏小/偏大，实验操作有一定难度。"
    else:
        sz_desc = "大小偏离理想范围，实验设计需特殊考虑。"

    if nov >= 10:
        nov_desc = "极度新颖，几乎未被系统研究（PubMed <= 20篇）。"
    elif nov >= 8:
        nov_desc = "非常新颖，仅有少数基础研究。"
    elif nov >= 6:
        nov_desc = "较新颖，有一定研究但存在未探索领域。"
    else:
        nov_desc = "有一定研究基础，但仍存在niche空间。"
    ...
    nov_label = "<= 20 -> 10" if pubmed_strict <= 20 else ("21-40 -> 8" if pubmed_strict <= 40 else ("41-60 -> 6" if pubmed_strict <= 60 else ("61-80 -> 4" if pubmed_strict <= 80 else ("81-100 -> 2" if pubmed_strict <= 100 else ">100 -> REJECTED"))))

    # Build cross-validation details
    cross_details = []
    if pdb_list and plddt > 0:
        cross_details.append("PDB + AlphaFold 双源验证: +0.5")
    has_multi_nuc = 0
    if has_uni and subcell:
        has_multi_nuc += 1
    if hpa_main:
        has_multi_nuc += 1
    if go_cc:
        has_multi_nuc += 1
    if has_multi_nuc >= 2:
        cross_details.append(f"多库定位一致 ({has_multi_nuc}源): +0.5")
    elif has_multi_nuc == 1:
        cross_details.append("多库定位一致: +0 (仅1源)")
    else:
        cross_details.append("多库定位一致: +0")
    if string_p and intact_p:
        cross_details.append("STRING + IntAct 双源验证: +0.5")
    else:
        cross_details.append("STRING + IntAct 双源验证: +0")
    if domain_text and "暂无" not in domain_text and plddt > 70:
        cross_details.append("结构域 + AlphaFold 质量: +0.5")
    else:
        cross_details.append("结构域 + AlphaFold 质量: +0")
    if len(pdb_list) >= 3:
        cross_details.append("PDB 多条目覆盖 (>=3): +1.0")
    else:
        cross_details.append("PDB 多条目覆盖: +0")
    ...
    func_lines = []
    if function_text:
        for fn_text in function_text:
            if isinstance(fn_text, dict):
                func_lines.append(fn_text.get("text", str(fn_text)))
            else:
                func_lines.append(str(fn_text))
    func_display = "\n".join(func_lines[:5]) if func_lines else "暂无功能注释（UniProt未提供详细功能描述）"

    # STRING partner count for table
    string_n = len(string_p)
    intact_n = len(intact_p)

    report = f"""---
type: protein-evaluation
gene: "{gene}"
date: 2026-06-03
tags: [protein-scout, {"rejected" if rejected else "nuclear-protein"}, evaluation, recovery, re-evaluation]
status: {status}
---
...
| 维度 | 得分 | 满分 | 加权后 | 关键证据摘要 |
|------|------|------|--------|-------------|
| 核定位特异性 | {nuc}/10 | x4 | {nuc*4} | HPA: {hpa_loc_text[:50]}; UniProt: {subcell_text[:50]} |
| 蛋白大小 | {sz}/10 | x1 | {sz} | {length_aa if length_aa else "未知"} aa / {mass} kDa |
| 研究新颖性 | {nov}/10 | x5 | {nov*5} | PubMed strict={pubmed_strict} 篇 ({nov_label}) |
| 三维结构 | {struct}/10 | x3 | {struct*3} | AlphaFold v{af_version} pLDDT={round(plddt,1)}; PDB: {pdb_text[:35]} |
| 调控结构域 | {dom}/10 | x2 | {dom*2} | {domain_text[:55]} |
| PPI 网络 | {ppi}/10 | x3 | {ppi*3} | STRING {string_n} partners; IntAct {intact_n} interactions |
...
- STRING partners: {string_n}，IntAct interactions: {intact_n}
- 调控相关比例: {len(reg)} / {min(string_n, 20)} = {reg_ratio:.0f}%
...
| PPI | STRING + IntAct | {string_n} + {intact_n} interactions | {"数据充分" if string_p and intact_p else "数据有限"} |
...
**推荐等级**: {"(REJECTED)" if rejected else chr(11088) * max(1, min(5, round(norm/20)))} {"(REJECTED)" if rejected else ""}
...
2. {"AlphaFold 预测质量一般（pLDDT=" + str(round(plddt,1)) + "），需要更多实验结构验证" if plddt < 70 and plddt > 0 else "结构数据质量可接受" if plddt >= 70 else "结构数据暂缺"}
...
- STRING: https://string-db.org/network/9606.{gene}
...
print("Generating 25 full re-evaluation reports (/180 format, recovery)...")
print("=" * 60)
results = []
for gene in GENES:
    try:
        ...
        for line in report.split("\n"):
            if "核定位特异性" in line and "|" in line:
                parts = line.split("|")
                if len(parts) >= 3:
                    try:
                        nuc_score = int(parts[2].strip().split("/")[0])
                    except:
                        pass
                break
        pubmed_strict = 0
        for line in report.split("\n"):
            if "PubMed strict count" in line and "|" in line:
                parts = line.split("|")
                if len(parts) >= 3:
                    try:
                        pubmed_strict = int(parts[2].strip())
                    except:
                        pass
                break
        rejected = nuc_score <= 3 or pubmed_strict > 100
        status = "OK" if size > 3000 else "SHORT"
        tag = "REJECTED" if rejected else "SCORED"
        print(f"  {gene:12s} -> {dest:20s}/{gene}/{gene}-evaluation.md ({size:5d} bytes) [{tag}] [{status}]")
        results.append({"gene": gene, "dest": dest, "size": size, "rejected": rejected, "status": status})
    except Exception as e:
        print(f"  {gene:12s} -> ERROR: {e}")
        import traceback
        traceback.print_exc()
        results.append({"gene": gene, "dest": "ERROR", "size": 0, "rejected": True, "status": f"ERROR: {e}"})
...
print(f"Summary: {ok} OK, {short} SHORT, {err} ERROR")
print(f"Scored: {scored}, Rejected: {rej}")
